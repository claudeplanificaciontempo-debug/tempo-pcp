...
window.__W={writes:[],deny:null}; // registro de escrituras y rechazo simulado de la base (RLS)
function builder(t){const st={t,op:'select',filters:[],single:false,payload:null,ids:null};
  const b={select(){return b},order(){return b},range(){return b},limit(){return b},eq(k,v){st.filters.push([k,v]);return b},single(){st.single=true;return b},
    ...
    then(res,rej){let rows=tabla(t);try{
      ...
      if(st.op==='upsert'){__W.writes.push({t,op:'upsert',n:(st.payload||[]).length});if(__W.deny&&__W.deny(t))return res({data:null,error:{message:'new row violates row-level security policy for table "'+t+'"'}});(st.payload||[]).forEach(r=>{const i=rows.findIndex(x=>x.id===r.id);const c=JSON.parse(JSON.stringify(r));if(i<0)rows.push(c);else rows[i]=c});return res({data:null,error:null})}
      if(st.op==='delete'){__W.writes.push({t,op:'delete',n:(st.ids||[]).length});if(__W.deny&&__W.deny(t))return res({data:null,error:{message:'permission denied for table '+t}});DB[t]=rows.filter(r=>!(st.ids||[]).includes(r.id));return res({data:null,error:null})}
