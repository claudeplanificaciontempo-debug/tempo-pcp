...
async function __run(){try{LISTO=true;}catch(e){}try{__R.prevFuzz=localStorage.__fuzz||'';__R.prevFase=localStorage.__fase||'';localStorage.__fuzz='';localStorage.__fase=''}catch(e){}
  ...
  {const antes=__R.errors.length;const tareaRows=window.__tareaRows||[];const al=window.alert;window.alert=()=>{};
   ...
   {PERFIL=adminP0();const ym=hoy().slice(0,7);PM.mes=ym;PM.paso=1;NIVUI.area='corte';NIVUI.esc={};SIM={on:false,ym:null,rec:null,cambios:{},motivo:''};const bakAj=JSON.stringify(S.params.ajustesCap||null);page='plan';render();const pg=()=>document.getElementById('p-plan');let h=pg().innerHTML;
    __check('PN: la pantalla se llama «Planificar el mes», tiene los dos pasos como chips y el paso 1 trae, en orden, el calendario de días, «Nivelación» (ya no «¿Alcanza la capacidad?») con los recuadros por área y, debajo, las personas SOLO del centro elegido (Corte); el paso 2 queda en el DOM pero oculto',/<h2>Planificar el mes/.test(h)&&/1 · Nivelación/.test(h)&&/2 · Plan mensual/.test(h)&&h.indexOf('id="plan-cal"')<h.indexOf('<h3>Nivelación')&&h.indexOf('<h3>Nivelación')<h.indexOf('Personas en Corte')&&!/Alcanza la capacidad/.test(h)&&!/Personas en Confección/.test(h)&&h.indexOf("NIVUI.area='corte'")<h.indexOf('Personas en Corte')&&/Saldo por/.test(h)&&pg().querySelector('#plan-paso1').style.display!=='none'&&pg().querySelector('#plan-paso2').style.display==='none'&&/Resumen del mes/.test(pg().querySelector('#plan-paso2').innerHTML)&&document.querySelector('nav a[data-p="plan"]').textContent.trim()==='Planificar el mes');
    ...
    __check('PN: en el paso 2, «Agregar órdenes al plan» ofrece también lo pendiente de meses anteriores (casilla marcada por defecto)',PMADD.incluirAnt!==false&&/Lo pendiente de meses anteriores/.test(pg().innerHTML));
    ...
    {const cards=[...pg().querySelectorAll('.niv-card')];const grupos=[...pg().querySelectorAll('.niv-grupo')];const tit=grupos.map(g=>g.querySelector('.niv-grupo-t').textContent.trim());
     ...
     const okEstado=cards.every(c=>{const a=nivUIArea(c.dataset.area);const r=nivUICalcular(a.id);const est=a.porDias?'dias':r.estado;return c.classList.contains('est-'+est)&&!!c.querySelector('.niv-pill')});
  ...
  {const antes=__R.errors.length;const al=window.alert;let av="";window.alert=m=>{av=String(m)};const adminP=PERFIL;
   ...
   {const areas=nivUIAreas();const pro=S.centros.filter(c=>c.area==="pro"&&c.activo!==false).length;
    ...
    __check("N3: cada recuadro trae saldo, fecha final (o por días / sin saldo) y estado con ícono en su pastilla",areas.every(a=>h.includes("NIVUI.area='"+a.id+"'"))&&(h.match(/class="niv-card /g)||[]).length===areas.length&&(h.match(/>saldo</gi)||[]).length===areas.length&&(h.match(/class="niv-pill /g)||[]).length===areas.length&&/[✓⚠✕?] (llega|riesgo|déficit|falta un dato)/.test(h)&&[...document.querySelectorAll("#p-nivelacion .niv-card")].every(c=>c.classList.contains("vacia")||c.classList.contains("est-dias")||/termina|fin —/.test(c.textContent)));
   ...
   {const tabla=faseMapeo();const bak=tabla.map(r=>r.nivel);
    ...
    __check("N8: Maquila sin fases marcadas = «sin fases marcadas · dato faltante», nunca 0",fasesDeProcNivel("maquila").length>0||(rM0.sinFasesTela===true&&rM0.estado==="faltante"&&/sin fases marcadas/.test(document.querySelector("#p-nivelacion .niv-card[data-area=maquila]").innerHTML)&&/falta marcar en la tabla 1/.test(document.querySelector("#p-nivelacion .niv-card[data-area=maquila]").textContent)));   /* la tarjeta dice «sin fases marcadas» + «? falta un dato · falta marcar en la tabla 1 qué fases cuentan» */
