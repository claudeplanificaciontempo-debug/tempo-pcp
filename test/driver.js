...
async function __run(){try{LISTO=true;}catch(e){}try{__R.prevFuzz=localStorage.__fuzz||'';__R.prevFase=localStorage.__fase||'';localStorage.__fuzz='';localStorage.__fase=''}catch(e){}
  ...
  if(/captura=(niv|fases|cola|borrado|res|audit|ux)/.test(location.search)){TAREA=planT;const pr=window.prompt;window.prompt=()=>'APLICAR';aplicarTarea();window.prompt=pr;await __p(50);PERFIL={id:'cap',rol:'planificacion',nombre:'Planificación'};
    ...
    if(/captura=ux/.test(location.search)){const m=location.search.match(/captura=ux([a-z]+?)(f?)(q?)$/)||[];const p=m[1]||'ordenes';PERFIL=adminP0();FILT={};
      ...
      if(m[2])FILT[p]=true;
      if(p==='lote'){page='ordenes';ORDF.tab='rutas';render();const porCat={};rutaLoteCandidatas().forEach(o=>{const k=K(o.cat);if(!k||!k.padre)return;(porCat[k.id]=porCat[k.id]||[]).push(o)});const catId=Object.keys(porCat).sort((a,b)=>porCat[b].length-porCat[a].length)[0];RLOTE.fam=K(catId).padre;RLOTE.cat=catId;mRutaLote();const cl=centrosLote();const ag=cl.find(c=>c.editable&&!(RLOTE.cens||[]).includes(c.id)&&['estampado','bordado'].includes(c.id));if(ag)rutaLoteTog(ag.id);
  ...
  {const antes=__R.errors.length;const tareaRows=window.__tareaRows||[];const al=window.alert;window.alert=()=>{};
   ...
   /* RLT · ruta en lote (20-sep): la misma ruta para las órdenes que la comparten, cada una con el SAM de su categoría */
   {PERFIL=adminP0();RLOTE=RLOTE0();
    /* el tipo de producto con más órdenes por editar (mismo alcance que ordenesLote), donde Empaque se pueda dejar en todas */
    const porCat={};rutaLoteCandidatas().forEach(o=>{const k=K(o.cat);if(!k||!k.padre)return;(porCat[k.id]=porCat[k.id]||[]).push(o)});Object.keys(porCat).forEach(id=>{if(!porCat[id].every(o=>pasosProCompleta(o).includes('empaque')||!pasoHecho(o,'empaque')))delete porCat[id]});
    const catId=Object.keys(porCat).sort((a,b)=>porCat[b].length-porCat[a].length)[0];const kk=K(catId);
    RLOTE.fam=kk.padre;RLOTE.cat=catId;mRutaLote();const m=document.getElementById('modal');const ords=ordenesLote();
    __check('RLT: hay un tipo de producto con 3+ órdenes por editar para probar',ords.length>=3,JSON.stringify({cat:kk&&kk.n,n:ords.length}));
    const fuera=S.ordenes.filter(o=>!ords.includes(o)).slice(0,400).map(o=>[o.id,JSON.stringify([o.ruta,o.rutaCompleta,o.rutaConf,o.rutaEditada])]);
    __check('RLT: el modal acota por familia y tipo de producto y cuenta las mismas órdenes que ordenesLote (solo por editar, sin prenda terminada, de ese tipo)',m.querySelector('#rlote-n').textContent===String(ords.length)&&ords.length===porCat[catId].length&&ords.every(o=>!rutaConfirmada(o)&&!rutaNoAplica(o)&&K(o.cat).id===catId),JSON.stringify({modal:m.querySelector('#rlote-n').textContent,n:ords.length,g:porCat[catId].length}));
    ...
    /* agregar un centro editable que la ruta base no tiene */
    ...
    /* lo marcado a mano sobrevive a un cambio de filtro; con Enter en Buscar se aplica el texto; Cancelar deja todo limpio */
    {const censM=RLOTE.cens.slice();rutaLoteSet('cli','');__check('RLT: los centros marcados a mano no se pierden al cambiar un filtro',RLOTE.tocado&&RLOTE.cens.join()===censM.join());
     document.getElementById('rlote-q').value='zzz-no-existe';rutaLoteTexto();__check('RLT: Buscar se aplica con Enter / Filtrar (no al salir del campo) y acota el lote',RLOTE.q==='zzz-no-existe'&&ordenesLote().length===0&&document.getElementById('modal').querySelector('.foot .btn:not(.ghost)').disabled);
     document.getElementById('rlote-q').value='';rutaLoteTexto();__check('RLT: al vaciar Buscar vuelven las mismas órdenes',ordenesLote().length===ords.length);}
    const nb=S.bitacora.length;const nAud=(S.params.auditoriaCambios||[]).length;
    /* semántica del plan: entra si se marcó, no lo tenía, es editable y no está hecho; sale si no se marcó y lo tenía; lo hecho / no editable se conserva */
    ...
    __check('RLT: el plan entra/sale/conserva según lo marcado, lo hecho y lo editable; cuenta cambian + igual = todas; dice qué llega sin tiempo; no escribe nada',planOk&&plan.cambian+plan.igual===ords.length&&plan.mas[agregar.id]>0&&ords.every(o=>!rutaConfirmada(o))&&S.bitacora.length===nb,JSON.stringify({cambian:plan.cambian,igual:plan.igual,mas:plan.mas,sinT:plan.sinT,conservan:plan.hechosConservados}));
    /* motivo obligatorio */
    ...
    /* ruta vacía o sin Empaque: no se aplica en lote (regla fija) */
    ...
    __check('RLT: salen de «por editar» y de «Rutas por confirmar»; las órdenes fuera del alcance no se tocaron',ords.every(o=>!matchEdicionOrd(o,'porEditar')&&!rutasPorDefinir().includes(o))&&fuera.every(([id,j])=>{const o=S.ordenes.find(x=>x.id===id);return !!o&&JSON.stringify([o.ruta,o.rutaCompleta,o.rutaConf,o.rutaEditada])===j}));
    /* ya confirmadas: fuera del lote salvo que se pida; y un paso hecho se conserva aunque se desmarque */
    __check('RLT: tras aplicar, el estado del lote queda limpio (filtros, casilla «se pisan», centros)',!RLOTE.fam&&!RLOTE.cat&&!RLOTE.incConf&&RLOTE.cens===null);
    RLOTE.fam=kk.padre;RLOTE.cat=catId;RLOTE.cens=null;mRutaLote();__check('RLT: las que acaban de confirmarse ya no entran al lote; con «incluir confirmadas» sí',ordenesLote().length===0&&(RLOTE.incConf=true,ordenesLote().length===ords.length),JSON.stringify({sin:ordenesLote().length}));
    ...
    if(filaConf&&pasosProCompleta(oH).includes('corte')){const bakH=JSON.stringify(oH);aplicarFaseSistema(oH,filaConf.fase);RLOTE.incConf=true;RLOTE.q=oH.op;RLOTE.cens=null;mRutaLote();
     ...
     Object.assign(oH,JSON.parse(bakH));}else __check('RLT: hay una orden con corte en la ruta para probar la conservación de pasos hechos',false,JSON.stringify({filaConf:!!filaConf,ruta:pasosProCompleta(oH)}));
    /* cierre en piso: la misma puerta que el motor (pasoHecho), no solo la fase */
    ...
    __check('RLT: tras aplicar y al cancelar el estado del lote queda limpio (sin «incluir confirmadas» ni filtros arrastrados)',(RLOTE.incConf=true,RLOTE.q='x',cancelarRutaLote(),!RLOTE.incConf&&RLOTE.q===''&&RLOTE.cens===null&&!RLOTE.tocado));
    cerrar();RLOTE=RLOTE0();PLAN=null;PLAN_ALL=null;}
