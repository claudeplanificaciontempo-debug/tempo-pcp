...
async function __run(){try{LISTO=true;}catch(e){}try{__R.prevFuzz=localStorage.__fuzz||'';__R.prevFase=localStorage.__fase||'';localStorage.__fuzz='';localStorage.__fase=''}catch(e){}
  ...
  {const antes=__R.errors.length;const tareaRows=window.__tareaRows||[];const al=window.alert;window.alert=()=>{};
   ...
   {PERFIL=adminP0();const ym=hoy().slice(0,7);PM.mes=ym;PM.paso=1;NIVUI.area='corte';NIVUI.esc={};SIM={on:false,ym:null,rec:null,cambios:{},motivo:''};const bakAj=JSON.stringify(S.params.ajustesCap||null);page='plan';render();const pg=()=>document.getElementById('p-plan');let h=pg().innerHTML;
    __check('PN: la pantalla se llama «Planificar el mes», tiene los dos pasos como chips y el paso 1 trae, en orden, el calendario de días, ¿Alcanza la capacidad? con los recuadros por área y, debajo, las personas SOLO del centro elegido (Corte); el paso 2 queda en el DOM pero oculto',/<h2>Planificar el mes/.test(h)&&/1 · Nivelación/.test(h)&&/2 · Plan mensual/.test(h)&&h.indexOf('id="plan-cal"')<h.indexOf('¿Alcanza la capacidad?')&&h.indexOf('¿Alcanza la capacidad?')<h.indexOf('Personas en Corte')&&!/Personas en Confección/.test(h)&&h.indexOf("NIVUI.area='corte'")<h.indexOf('Personas en Corte')&&/Saldo por/.test(h)&&pg().querySelector('#plan-paso1').style.display!=='none'&&pg().querySelector('#plan-paso2').style.display==='none'&&/Resumen del mes/.test(pg().querySelector('#plan-paso2').innerHTML)&&document.querySelector('nav a[data-p="plan"]').textContent.trim()==='Planificar el mes');
