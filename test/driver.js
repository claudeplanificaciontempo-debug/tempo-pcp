...
async function __run(){try{LISTO=true;}catch(e){}try{__R.prevFuzz=localStorage.__fuzz||'';__R.prevFase=localStorage.__fase||'';localStorage.__fuzz='';localStorage.__fase=''}catch(e){}
  ...
  __check('centrosPorOrden por defecto: estampado, bordado, lavado, plancha + sublimado, apliques, calandrado, cordones (pasos nuevos 20-sep)',JSON.stringify(centrosPorOrden())===JSON.stringify(['estampado','bordado','lavado','plancha','sublimado','apliques','calandrado','cordones']),JSON.stringify(centrosPorOrden()));
  ...
  if(/captura=(niv|fases|cola|borrado|res|audit|ux)/.test(location.search)){TAREA=planT;const pr=window.prompt;window.prompt=()=>'APLICAR';aplicarTarea();window.prompt=pr;await __p(50);PERFIL={id:'cap',rol:'planificacion',nombre:'Planificación'};
    ...
    /* captura=borrado1: el panel y la confirmación · borrado2: la pantalla final después de borrar (sobre la base simulada) */
    if(/captura=borrado/.test(location.search)){PERFIL={id:"cap",rol:"admin",nombre:"Administrador"};const oA=S.ordenes.find(o=>abiertaDe(o)&&o.op);S.params.pendPospuestos={["x-"+oA.id]:{hasta:"2026-12-31",motivo:"p"}};S.params.planMes={"2026-10":{oids:[oA.id]}};S.params.alertasCompras=[{id:"ac",oid:oA.id,op:oA.op}];S.avance[oA.id]={centros:{corte:5}};
    ...
    if(/captura=ux/.test(location.search)){const m=location.search.match(/captura=ux([a-z]+?)(f?)(q?)$/)||[];const p=m[1]||'ordenes';PERFIL=adminP0();FILT={};
      ...
      page=p;render();if(m[3]){const a=document.querySelector('#p-'+p+' .pagehead .ayuda-cab');if(a)a.classList.add('abierto')}
  ...
  {const antes=__R.errors.length;const adminP=PERFIL;
   ...
   page='config';CONF.tab='borrado';render();const pc=document.getElementById('p-config').innerHTML;__check("Configuración → Borrado: panel con conteos y UN solo botón (17-sep)",pc.includes('Borrar datos de prueba')&&pc.includes('registros de avance de piso')&&(pc.match(/onclick="mBorrar\(/g)||[]).length===1&&!/mBorrar\('/.test(pc));
   const nO=S.ordenes.length;mBorrar();const fr=document.getElementById('borrar-frase');__check("Borrado: pide la palabra BORRAR escrita, con el conteo de órdenes a la vista",!!fr&&document.getElementById('modal').innerHTML.includes('escribe exactamente <b>BORRAR</b> en este recuadro')&&document.getElementById('modal').innerHTML.includes('Órdenes: <b>'+num(nO)+'</b>'));
   const alertPrev=window.alert;let al='';window.alert=m=>al=m;fr.value='borrar todo';await ejecutarBorrado();__check("Borrado: palabra incorrecta no borra nada",S.ordenes.length===nO&&/No se borró nada/.test(al)&&/BORRAR TODO/.test(al));window.alert=alertPrev;cerrar();
   PERFIL={rol:'piso',modo:'editar',nombre:'P'};let al2='';window.alert=m=>al2=m;mBorrar();__check("Borrado: sin permiso config no abre",/administrador/.test(al2));window.alert=alertPrev;PERFIL=adminP;
  ...
  {const fe=f=>faseEstado(f);const h=f=>fe(f).hechos.slice().sort().join(',');
   ...
   __check('4 Calidad Produccion: por tabla (grupo corte) → tela lista, nada de producción hecho (salvo Calandrado, que va antes de corte)',fe('4 Calidad Produccion').lista&&fe('4 Calidad Produccion').hechos.filter(c=>c!=='calandrado').length===0&&!fe('4 Calidad Produccion').sinFila,JSON.stringify(fe('4 Calidad Produccion').hechos));
  ...
  {const antes=__R.errors.length;const adminP=PERFIL;window.confirm=()=>true;const a0=window.alert;window.alert=()=>{};
   ...
   {planMesQuitar(ym,oT.id);const h2=document.getElementById('p-plan').innerHTML;const enCand=(()=>{const i=h2.indexOf('<h3>Agregar órdenes al plan');return h2.slice(i).includes(esc(oT.op))})();
    ...
    {const oQ=mk('WH/TEST-PB-Q','4CD Ensamble');oQ.fecha=ym+'-25';planMesAgregar(ym,[oQ.id]);planMesQuitar(ym,oQ.id);const q1=planMesQuitadas(ym).has(oQ.id)&&!planMesOids(ym).has(oQ.id)&&!planMesOidsTot(ym).has(oQ.id);
     oQ.fecha=sig+'-05';PLAN=null;PLAN_ALL=null;planMesAgregar(ym,[oQ.id]);const q2=planMesOids(ym).has(oQ.id)&&!planMesQuitadas(ym).has(oQ.id)&&planMesOidsTot(ym).has(oQ.id);
  ...
  {const antes=__R.errors.length;const adminP=PERFIL;const ym=hoy().slice(0,7);PM.mes=ym;
   ...
   {const dN=new Date(ym+'-15T12:00:00');dN.setMonth(dN.getMonth()+1);const sig=dN.toISOString().slice(0,7);oCand.fecha=sig+'-10';oNoCand.fecha=ym+'-28';}   /* candidata = entrega el mes siguiente (excepción); no candidata = entrega en el mes (ya en la base) */
  ...
  {const antes=__R.errors.length;const adminP=PERFIL;window.confirm=()=>true;const alerts=[];const a0=window.alert;window.alert=m=>alerts.push(String(m));
   ...
   {const r1=mkR('WH/REF-1',['corte','modulos'],null,{ref:'REF-X'});const r2=mkR('WH/REF-2',['corte'],null,{ref:'REF-X'});
    ...
    const h=document.getElementById('p-ordenes').innerHTML;
    __check("RU3: la pestaña Rutas es UN recuadro: buscador, agrupador común, chips por confirmar / confirmadas / todas, acciones sobre lo marcado y lo de Odoo plegado en «Más herramientas»",/Por confirmar <span class="mut">/.test(h)&&/Definir ruta para las marcadas/.test(h)&&/Confirmar como están/.test(h)&&/Qué dice Odoo hoy/.test(h)&&/Más herramientas/.test(h)&&/data-q="RUT.q"/.test(h)&&/setNivelGRP\('rut'/.test(h)&&!/Por definir <span class="note">/.test(h));
    RUT.ver='conf';{const nv=grpSt('rut').niveles;grpSt('rut').sembrado=true;grpSt('rut').niveles=[];render();   /* sin agrupar: las filas se ven (agrupadas, los grupos nacen plegados) */
    __check("RU3: en «Confirmadas» cada fila trae origen, quién y cuándo",/Odoo OT|persona<\/span>/.test(document.getElementById('p-ordenes').innerHTML));grpSt('rut').niveles=nv;}RUT.ver='pend';
  ...
  {const antes=__R.errors.length;const adminP=PERFIL;window.confirm=()=>true;const a0=window.alert;window.alert=()=>{};
   ...
   {const P=S.ordenes.length?programar():{pro:[],ordenes:{}};const lun=lunesDe(hoy()),dom=dsum(lun,6);
    ...
    __check("B2: Estampado (serigrafía) trae Estampado, Etiquetas y los pasos nuevos del 20-sep (Sublimado, Apliques, Calandrado)",!!rE&&rE.filas.map(f=>f.c).sort().join()===['estampado','etiquetas','sublimado','apliques','calandrado'].sort().join(),rE&&rE.filas.map(f=>f.c).join());
  ...
  {const antes=__R.errors.length;const adminP=PERFIL;window.confirm=()=>true;const a0=window.alert;window.alert=()=>{};
   ...
   {const k=S.categorias.find(x=>x.padre&&!tieneHojaLMO(x)&&!x.samManda&&(K(x.padre)||{}).minEstConf==null);   /* sin hoja, sin un SAM que mande (el del 21-sep gana a todo) y sin minuto heredado del padre */
    if(!k)__check('ME: hay alguna categoría sin hoja de operaciones para probar',false);
    else{
     delete k.minEstConf;
     __check("ME: sin valor cargado, la categoría sigue en 0: no se inventa nada",minEstimadoConf(k)===null&&samPorCentro(k).modulos===undefined,JSON.stringify({k:nombreCat(k),me:minEstimadoConf(k),sp:samPorCentro(k)}));
  ...
  {const antes=__R.errors.length;const tareaRows=window.__tareaRows||[];const al=window.alert;window.alert=()=>{};
   ...
   /* MA · maestro de productos (19-sep): origen y proveedor POR PRODUCTO; recálculo de telas sin recargar Odoo; tabla 12 por proveedor */
   {const maestroRows=await (await fetch('fixtures/maestro_rows.json')).json();const H=maestroRows[0];const col=n=>H.indexOf(n);
    const p0=planTarea(tareaRows,"TAREA_PARTE2.xlsx");TAREA=p0;aplicarTarea();await __p(50);
    delete S.params.maestroProductos;delete S.params.maestroCarga;S.params.diasProveedor=(S.params.diasProveedor||[]).filter(r=>r.origen!=='maestro de productos');
    const sinClasAntes=Object.keys(p0.telaSinClasificar);const nSinClasAntes=S.ordenes.filter(o=>abiertaDe(o)&&(o.telaSinClasificar||[]).length).length;
    const pm=planMaestro(maestroRows,"Maestro_Productos.xlsx");
    __check("MA: el maestro se lee por nombre de columna: 3.276 productos, TEMPO/propia, con proveedor y sin proveedor suman el total",!!pm&&pm.filas===3276&&pm.tempo+pm.conProv+pm.sinProv===pm.filas&&pm.tempo>1400&&pm.sinProv>800&&pm.columnas.prov&&pm.columnas.ruta&&pm.columnas.propia,JSON.stringify(pm&&{f:pm.filas,t:pm.tempo,c:pm.conProv,s:pm.sinProv}));
    __check("MA: el cruce con las órdenes cargadas dice cuántos códigos están en el maestro y cuántas telas quedan con origen",pm.cruce.codigos>500&&pm.cruce.enMaestro>=pm.cruce.codigos-20&&pm.cruce.telas>300&&pm.cruce.telasConOrigen>pm.cruce.telas*0.85&&pm.cruce.telasSinProv.length>20,JSON.stringify(pm.cruce&&{c:pm.cruce.codigos,e:pm.cruce.enMaestro,t:pm.cruce.telas,o:pm.cruce.telasConOrigen,s:pm.cruce.telasSinProv.length}));
    __check("MA: la vista previa muestra los números (nuevos, TEMPO, sin proveedor) y las telas por tipo de Odoo",(()=>{const h=resumenMaestroHTML(pm);return new RegExp("<b>"+num(pm.nuevos)+"<\\/b><\\/td><td>productos <b>nuevos<\\/b>").test(h)&&new RegExp("<b>"+num(pm.sinProv)+"<\\/b><\\/td><td><b>sin proveedor<\\/b>").test(h)&&/NUEVOS TEMPO/.test(h)&&/Cruce con las órdenes cargadas/.test(h)})());
    __check("MA: un producto sin proveedor y sin marca propia no da origen (decide la tabla 3); TEMPO da PROPIA; proveedor da EXTERNA",(()=>{const f=Object.values(pm.productos).find(x=>!x.prov&&!x.propia),t=Object.values(pm.productos).find(x=>x.propia),e=Object.values(pm.productos).find(x=>x.prov&&!x.propia&&!esTempoProv(x.prov));maestroProductos()[f.cod]=f;maestroProductos()[t.cod]=t;maestroProductos()[e.cod]=e;const r=origenPorMaestro(f.cod)===null&&origenPorMaestro(t.cod)==="PROPIA"&&origenPorMaestro(e.cod)==="EXTERNA"&&provPorMaestro(e.cod)===e.prov;delete maestroProductos()[f.cod];delete maestroProductos()[t.cod];delete maestroProductos()[e.cod];return r})());
    /* aplicar (sin recalcular todavía) */
    const nb=S.bitacora.length;const cf=window.confirm;window.confirm=()=>false;MAESTRO=pm;aplicarMaestro();window.confirm=cf;await __p(50);
    __check("MA: al aplicar, el maestro queda guardado por código con nombre, unidad, proveedor, categoría y propia; bitácora y registro de cargas",Object.keys(maestroProductos()).length===pm.filas&&(()=>{const x=Object.values(pm.productos).find(x=>x.prov&&!x.propia);const m=maestroProductos()[x.cod];return m&&m.prov===x.prov&&m.udm===x.udm&&m.ruta===x.ruta&&m.propia===false})()&&S.bitacora.slice(nb).some(b=>/Maestro de productos/.test(b.t))&&(S.cargas||[]).some(c=>c.tipo==="maestro")&&/productos/.test(resumenCargaTxt((S.cargas||[]).filter(c=>c.tipo==="maestro").slice(-1)[0])),Object.keys(maestroProductos()).length);
    __check("MA: cada proveedor nuevo entra a la tabla de días de entrega con el estimado general marcado «estimado» (como los demás), TEMPO no",(()=>{const D=diasProveedor();const nuevos=D.filter(r=>r.origen==="maestro de productos");return nuevos.length>20&&nuevos.every(r=>r.dias===DIAS_PROV_EST&&r.estimado===true&&r.prov&&!esTempoProv(r.prov))&&!D.some(r=>esTempoProv(r.prov)&&r.origen==="maestro de productos")})());
    /* un producto que vuelve SIN proveedor conserva el que tenía */
    {const x=Object.values(pm.productos).find(x=>x.prov&&!x.propia&&!esTempoProv(x.prov));const rows2=[maestroRows[0]].concat(maestroRows.slice(1).map(r=>{if(String(r[col("Referencia Interna")])!==x.cod)return r;const y=r.slice();y[col("Proveedor")]=null;return y}));
     const pm2=planMaestro(rows2,"Maestro2.xlsx");__check("MA: en la vista previa, un producto que viene sin proveedor y aquí ya lo tenía cuenta como «se conserva»",pm2.conservaProv===1&&pm2.cambiaProv.length===0);
     window.confirm=()=>false;MAESTRO=pm2;aplicarMaestro();window.confirm=cf;await __p(30);__check("MA: y tras aplicar conserva el proveedor de aquí, marcado «conservado»",maestroProductos()[x.cod].prov===x.prov&&maestroProductos()[x.cod].provConservado===true);}
    /* origen por producto manda sobre la categoría: una tela LYCRA (SIN CLASIFICAR por tabla 3) con proveedor en el maestro es externa */
    {const o=S.ordenes.find(o=>abiertaDe(o)&&(o.materiales||[]).some(m=>m.clasif==="tela"&&m.origen==="SIN CLASIFICAR"&&origenPorMaestro(m.cod)==="EXTERNA"));
     __check("MA: hay órdenes con tela SIN CLASIFICAR por categoría que el maestro resuelve como externa",!!o);
     if(o){const m=o.materiales.find(m=>m.clasif==="tela"&&m.origen==="SIN CLASIFICAR"&&origenPorMaestro(m.cod)==="EXTERNA");const dm=dimensionesTela(m);const l=lineasTelaDe([m])[0];
      __check("MA: dimensionesTela y lineasTelaDe usan el origen del maestro (externa, a pedir, con su proveedor)",dm.produce==="externa"&&dm.disp==="pedir"&&dm.prov===provPorMaestro(m.cod)&&l.origen==="EXTERNA"&&l.produce==="externa",JSON.stringify(dm));}}
    /* recálculo de telas sin recargar Odoo */
    const prev=recalcularTelasOrdenes(false);
    __check("MA: la vista previa del recálculo cuenta órdenes que cambian sin tocar nada",prev.n>500&&prev.cambiadas>0&&S.ordenes.filter(o=>abiertaDe(o)&&(o.telaSinClasificar||[]).length).length===nSinClasAntes,JSON.stringify({n:prev.n,c:prev.cambiadas,g:prev.ganaron,p:prev.perdieron}));
    {const o=S.ordenes.find(o=>abiertaDe(o)&&prev.det.includes(o.op)&&(o.telas||[]).length);const tl=o&&o.telas[0];if(tl){tl.faltaConf={v:"nada",u:"ma",ts:new Date().toISOString(),antes:"tintura"}}
     const nb2=S.bitacora.length;const r=recalcularTelasOrdenes(true);await __p(50);
     __check("MA: al aplicar cambian las mismas órdenes, queda en bitácora y baja el número de órdenes con tela sin clasificar",r.cambiadas===prev.cambiadas&&S.bitacora.slice(nb2).some(b=>/Telas recalculadas/.test(b.t))&&S.ordenes.filter(o=>abiertaDe(o)&&(o.telaSinClasificar||[]).length).length<nSinClasAntes,JSON.stringify({antes:nSinClasAntes,ahora:S.ordenes.filter(o=>abiertaDe(o)&&(o.telaSinClasificar||[]).length).length}));
     if(tl){const o2=S.ordenes.find(x=>x.id===o.id);const t2=(o2.telas||[]).find(t=>t.tela===tl.tela);__check("MA: lo confirmado a mano («qué le falta») se conserva en la tela recalculada",!!t2&&t2.faltaConf&&t2.faltaConf.v==="nada"&&faltaTela(t2)==="nada");}
     const r2=recalcularTelasOrdenes(false);__check("MA: el recálculo es idempotente: la segunda pasada no cambia nada",r2.cambiadas===0,r2.cambiadas);}
    /* la carga de tareas ya usa el maestro: menos tipos SIN CLASIFICAR que antes */
    {const p1=planTarea(tareaRows,"TAREA_MA.xlsx");__check("MA: al volver a cargar las tareas, el maestro reduce los tipos de tela «sin clasificar» y la carga usa la misma función de telas (armarTelasDe)",Object.keys(p1.telaSinClasificar).length<sinClasAntes.length&&/armarTelasDe\(/.test(String(planTarea))&&/armarTelasDe\(/.test(String(recalcularTelasOrdenes)),sinClasAntes.length+" → "+Object.keys(p1.telaSinClasificar).length);
     TAREA=p1;aplicarTarea();await __p(50);}
    /* tabla 12 agrupada por proveedor, sin proveedor primero */
    page="config";CONF.tab="ordenes2";CONF.qProd="";CONF.prodSoloOrdenes=true;render();{const h=document.getElementById("p-config").innerHTML;const i1=h.indexOf("<b>(sin proveedor)</b>"),i2=h.indexOf("<b>TEMPO</b>");
     __check("MA: la tabla 12 tiene los botones de maestro y recálculo, agrupa por proveedor y «sin proveedor» va primero",/mCargarMaestro\(/.test(h)&&/mRecalcularTelas\(/.test(h)&&i1>0&&i2>i1&&/productos en el maestro/.test(h)&&/Proveedor \(maestro\)/.test(h));}
    CONF.prodSoloOrdenes=false;
   ...
   {delete S.params.faltaPorOrigen;const t=faltaPorOrigen();
    ...
    {const maestroRows=await (await fetch('fixtures/maestro_rows.json')).json();const pm=planMaestro(maestroRows,"Maestro_MB.xlsx");const cf=window.confirm;window.confirm=()=>false;MAESTRO=pm;aplicarMaestro();window.confirm=cf;await __p(30);
     ...
     delete S.params.maestroProductos;delete S.params.maestroCarga;S.params.diasProveedor=(S.params.diasProveedor||[]).filter(r=>r.origen!=="maestro de productos");S.cargas=(S.cargas||[]).filter(c=>c.tipo!=="maestro");
     const pz=planTarea(tareaRows,"TAREA_PARTE2.xlsx");TAREA=pz;aplicarTarea();await __p(50);}}
   /* OO · orden de las operaciones dentro de la prenda (hoja de producción, 19-sep) y segunda entrega de tiempos */
   {const ordRows=await (await fetch('fixtures/ordenops_rows.json')).json();
    S.operaciones.forEach(o=>{delete o.orden;delete o.ordenMeta;delete o.ordenObs;delete o.maqCorr;if(o.maqAntes){o.maq=o.maqAntes;delete o.maqAntes}});delete S.params.ordenOpsCarga;
    const po=planOrdenOps(ordRows,"OPERACIONES_PARA_LLENAR1.xlsx");
    __check("OO: la hoja se lee desde su fila de títulos: 595 operaciones, todas con número de orden",!!po&&po.filas===595&&po.sinOrden.length===0&&po.conOrden===po.calzan.length,JSON.stringify(po&&{f:po.filas,c:po.calzan.length,nc:po.noCalzan.length,so:po.sinOrden.length}));
    __check("OO: todas calzan con el catálogo por código (y familia en singular, con tolerancia a HODDIE/HOODIES); la fila con familia «0» se toma del bloque POLOS y se reporta; los códigos repetidos en una familia se desempatan por nombre",po.calzan.length===595&&(po.repetidas||[]).length<=20&&po.famCorregidas.length===1&&po.famCorregidas[0].familia==="POLOS"&&po.famCorregidas[0].cod==="ASEPÑ"&&po.noCalzan.length<=15,JSON.stringify({nc:po.noCalzan.slice(0,5),fc:po.famCorregidas}));
    __check("OO: la vista previa muestra los números y no hay correcciones de máquina en esta hoja",po.corrMaq.length===0&&new RegExp("<b>"+num(po.calzan.length)+"<\\/b><\\/td><td>calzan con el catálogo").test(resumenOrdenOpsHTML(po))&&/Por familia:/.test(resumenOrdenOpsHTML(po)));
    const kCR=S.categorias.find(x=>x.padre&&opsConfeccion(x).length>5);const antesOps=kCR?opsConfeccion(kCR):[];
    __check("OO: antes de aplicar, el balanceo de una categoría con hoja tiene todas las operaciones con orden provisional",!!kCR&&antesOps.length>5&&antesOps.every(x=>x.provisional),antesOps.length);
    const nb=S.bitacora.length;const cf=window.confirm;window.confirm=()=>true;ORDOPS=po;aplicarOrdenOps();window.confirm=cf;await __p(30);
    __check("OO: al aplicar, las operaciones guardan orden y meta, y queda en bitácora y en params",S.operaciones.filter(o=>o.orden!=null).length===po.calzan.length-(po.repetidas||[]).length&&S.operaciones.filter(o=>o.orden!=null).every(o=>o.ordenMeta&&o.ordenMeta.fuente==="hoja de producción")&&S.bitacora.slice(nb).some(b=>/Orden de las operaciones/.test(b.t))&&(S.params.ordenOpsCarga||{}).n===po.calzan.length);
    const desp=opsConfeccion(kCR);
    __check("OO: el balanceo usa el orden real: sin provisionales, ordenado por número de paso (empates en el orden del archivo)",desp.length===antesOps.length&&desp.every(x=>!x.provisional)&&desp.every((x,i)=>i===0||desp[i-1].orden<=x.orden)&&desp.some((x,i)=>i>0&&desp[i-1].orden!==x.orden),JSON.stringify(desp.slice(0,6).map(x=>x.orden+' '+x.n)));
    {const prov=S.operaciones.filter(o=>o.centro==='modulos'&&o.orden==null);__check("OO: las operaciones de confección que quedaron sin orden son pocas y el plan las contó",prov.length===(S.params.ordenOpsCarga||{}).quedanSinOrden,prov.length+" sin orden");}
    /* una hoja con una corrección de máquina y un cambio de número */
    {const H=ordRows.findIndex(r=>Array.isArray(r)&&r.some(x=>String(x||"")==="Código"));const hdr=ordRows[H];const iC=hdr.indexOf("Corrección de máquina"),iO=hdr.findIndex(x=>/^ORDEN/.test(String(x||"")));const rows2=ordRows.map((r,i)=>{if(i<=H)return r;const x=r.slice();if(String(x[1])==="TENCAM"&&String(x[0])==="CAMISETAS"){x[iC]="MESA DE TENDIDO";x[iO]=2}return x});
   ...
   {const p0=planTarea(tareaRows,"TAREA_PARTE2.xlsx");TAREA=p0;aplicarTarea();await __p(50);
    ...
    {ORDF.edicion="porEditar";ORDF.estado="plan";ORDF.q="";ORDF.fases=null;ORDF.tab="ord";GRP={};grpSt("ord").niveles=[];delete BUSQ["ORDF.q"];page="ordenes";render();const enLista=op=>{const c=document.querySelector("#p-ordenes [data-lista]");return !!c&&c.innerHTML.includes(esc(op))};
     ...
     if(o2){mOrden(o2.id);const m=document.getElementById("modal");const hm=m.innerHTML;
      ...
      __check("UI19: los cuatro bloques van abiertos (la usuaria los cierra) y el de Insumos dice cuántos componentes trae Odoo",[...m.querySelectorAll("details.ed-blk")].every(d=>d.open)&&/componentes de Odoo|en la ficha/.test(m.querySelectorAll("details.ed-blk")[3].querySelector("summary").textContent));
   ...
   /* RLT · ruta en lote (20-sep): la pestaña Rutas es UN recuadro con el agrupador común; se marca un grupo entero y se define la ruta para todas las marcadas */
   {PERFIL=adminP0();RLOTE=RLOTE0();RUT.sel=new Set();RUT.ver='pend';RUT.q='';delete BUSQ['RUT.q'];GRP={};grpSt('rut').niveles=['fam','hija'];page='ordenes';ORDF.tab='rutas';render();
    const pg=()=>document.getElementById('p-ordenes');
    __check('RLT: la lista de Rutas trae las abiertas cuya ruta se revisa y por defecto solo las por confirmar, agrupadas por familia y tipo de producto con casilla por grupo',rutasLista().length===rutasPorDefinir().length&&rutasLista().every(o=>!rutaConfirmada(o)&&!rutaNoAplica(o))&&pg().querySelectorAll('tr.grp-row').length>0&&/marcar el grupo/.test(pg().innerHTML)&&pg().querySelector('#rut-btn-def').disabled,JSON.stringify({lista:rutasLista().length,pend:rutasPorDefinir().length}));
    /* el grupo (tipo de producto) con más órdenes por confirmar donde Empaque se pueda dejar en todas */
    const porCat={};rutasLista().forEach(o=>{const k=K(o.cat);if(!k||!k.padre)return;(porCat[k.id]=porCat[k.id]||[]).push(o)});Object.keys(porCat).forEach(id=>{if(!porCat[id].every(o=>pasosProCompleta(o).includes('empaque')||!pasoHecho(o,'empaque')))delete porCat[id]});
    const catId=Object.keys(porCat).sort((a,b)=>porCat[b].length-porCat[a].length)[0];const kk=K(catId);const grupo=porCat[catId]||[];
    __check('RLT: hay un tipo de producto con 3+ órdenes por confirmar para probar',grupo.length>=3,JSON.stringify({cat:kk&&kk.n,n:grupo.length}));
    ...
    /* una fila suelta: marcar y desmarcar sin redibujar */
    {const o1=grupo[0];togSelRut(o1.id,false);__check('RLT: desmarcar una fila suelta descuenta sin redibujar la lista',RUT.sel.size===grupo.length-1&&pg().querySelector('#rut-sel-n').textContent===String(grupo.length-1));togSelRut(o1.id,true);}
    abrirRutaLote();const m=document.getElementById('modal');const ords=ordenesLote();
    __check('RLT: el modal toma exactamente las marcadas (abiertas cuya ruta se revisa) y no tiene filtros propios',m.querySelector('#rlote-n').textContent===String(ords.length)&&ords.length===grupo.length&&ords.every(o=>RUT.sel.has(o.id))&&!m.querySelector('select')&&/Órdenes marcadas/.test(m.innerHTML),JSON.stringify({modal:m.querySelector('#rlote-n').textContent,n:ords.length,g:grupo.length}));
    const base=rutaLoteBase(ords);__check('RLT: la ruta nueva arranca en la ruta más común del grupo y el paso 2 la lista con «usar»',base.length>0&&base.every(c=>m.querySelector('#rl-'+c)&&m.querySelector('#rl-'+c).checked)&&/usar<\/button>/.test(m.innerHTML),base.join(','));
    const cl=centrosLote();const agregar=cl.find(c=>c.editable&&!base.includes(c.id)&&['bordado','estampado','botones','plancha','etiquetas'].includes(c.id))||cl.find(c=>c.editable&&!base.includes(c.id));
    __check('RLT: el lote ofrece los mismos centros de producción que el editor por orden (lavado incluido; su modalidad sigue por orden) y hay un centro para agregar',cl.length===S.centros.filter(c=>c.area==='pro').length&&cl.some(c=>c.id==='lavado')&&!!agregar,agregar&&agregar.id);
    __check('RLT: las reglas fijas se avisan a la vista: sin Empaque o sin un centro «por defecto» sale el aviso; con la ruta base completa no',avisosRutaLote(base.filter(c=>c!=='empaque')).some(a=>/Empaque/.test(a))&&avisosRutaLote(base.concat(centrosRutaDefecto())).length===0,JSON.stringify(avisosRutaLote(base)));
    rutaLoteTog(agregar.id);if(!RLOTE.cens.includes('empaque'))rutaLoteTog('empaque');const plan=planRutaLote(ordenesLote(),RLOTE.cens);
    const nb=S.bitacora.length;const nAud=(S.params.auditoriaCambios||[]).length;
    const sel=new Set(RLOTE.cens);const planOk=plan.filas.every(f=>{const fe=faseEstado(f.o.fase,f.o);const tiene=pasosProCompleta(f.o);const enP=c=>(f.o.ruta||[]).some(p=>p.centro===c);return cl.every(c=>{const en=tiene.includes(c.id),want=sel.has(c.id);const fijo=!c.editable||pasoHecho(f.o,c.id,fe)||pasoFijoPorFase(f.o,c.id);
      if(fijo)return !f.mas.includes(c.id)&&!f.menos.includes(c.id)&&(f.conserv.includes(c.id)===(en&&!want))&&(f.despues.includes(c.id)===en);
      return (f.mas.includes(c.id)===(want&&!enP(c.id)))&&(f.menos.includes(c.id)===(!want&&en))&&(f.despues.includes(c.id)===want)&&new Set(f.despues).size===f.despues.length})});
    __check('RLT: el plan entra/sale/conserva según lo marcado, lo hecho, lo excluido por la fase y lo editable; cambian + igual = todas; dice qué llega sin tiempo; no escribe nada',planOk&&plan.cambian+plan.igual===ords.length&&plan.mas[agregar.id]>0&&ords.every(o=>!rutaConfirmada(o))&&S.bitacora.length===nb,JSON.stringify({cambian:plan.cambian,igual:plan.igual,mas:plan.mas,sinT:plan.sinT,conservan:plan.hechosConservados}));
    const al=window.alert;const alerts=[];window.alert=x=>alerts.push(String(x));const cf=window.confirm;window.confirm=()=>true;
    document.getElementById('rlote-motivo').value='';RLOTE.motivo='';const n0=aplicarRutaLote();
    __check('RLT: sin motivo no aplica nada',n0===0&&alerts.some(a=>/motivo/i.test(a))&&ords.every(o=>!rutaConfirmada(o)));
    PERFIL={id:'u-corte',rol:'corte',nombre:'Sup',modo:'editar'};document.getElementById('rlote-motivo').value='prueba lote';const n1=aplicarRutaLote();PERFIL=adminP0();
    __check('RLT: un perfil sin permiso de rutas no aplica el lote',n1===0&&ords.every(o=>!rutaConfirmada(o)));
    {const censB=RLOTE.cens.slice();document.getElementById('rlote-motivo').value='prueba';RLOTE.cens=[];alerts.length=0;const nv=aplicarRutaLote();const okV=nv===0&&alerts.some(a=>/vac/i.test(a));RLOTE.cens=censB.filter(c=>c!=='empaque');alerts.length=0;const ne=aplicarRutaLote();
     __check('RLT: una ruta vacía o que deje órdenes sin Empaque no se aplica en lote (regla fija: toda ruta termina en Empaque) y nada cambia',okV&&ne===0&&alerts.some(a=>/Empaque/.test(a))&&ords.every(o=>!rutaConfirmada(o)),JSON.stringify(alerts.slice(0,2)));RLOTE.cens=censB;}
    mRutaLote();{const m=document.getElementById('modal');const cens=m.querySelector('.rl-cens');const ruta=m.querySelector('.rl-ruta');const esperado=RLOTE.cens.slice().sort((a,b)=>ordenPaso(a)-ordenPaso(b)).map(nCen);
     __check('RLT: en el paso 3 los centros van a todo lo ancho (sin el tope de 420 px de .chips) en el orden del proceso, y debajo dice la ruta que quedará en ese orden',!!cens&&getComputedStyle(cens).maxWidth==='none'&&!!ruta&&/Ruta que quedará/.test(ruta.textContent)&&[...ruta.querySelectorAll('b')].map(b=>b.textContent).join('|')===esperado.join('|')&&[...cens.querySelectorAll('label')].map(l=>l.textContent.trim().replace(/\s*por defecto$/,''))[0]===nCen(centrosLote()[0].id),JSON.stringify({mw:cens&&getComputedStyle(cens).maxWidth,ruta:ruta&&ruta.textContent.trim(),esperado}));}
    document.getElementById('rlote-motivo').value='las '+kk.n+' llevan '+agregar.n+' (prueba)';const n2=aplicarRutaLote();window.alert=al;window.confirm=cf;
    const key=l=>l.slice().sort((a,b)=>ordenPaso(a)-ordenPaso(b)).join('→');
    __check('RLT: aplica a todas las marcadas: cada orden queda con la ruta que decía el plan (completa y pendiente coherentes, en orden de paso), confirmada (persona, nota «ruta en lote»), sellada y, si cambió, con rutaEditada etapa lote; la selección se limpia',n2===ords.length&&RUT.sel.size===0&&plan.filas.every(f=>{const o=f.o;const comp=key(pasosProCompleta(o));const pend=(o.ruta||[]).map(p=>p.centro).filter(c=>CE(c)&&CE(c).area==='pro');return comp===key(f.despues)&&pend.every(c=>f.despues.includes(c))&&f.mas.every(c=>pend.includes(c))&&!f.menos.some(c=>pend.includes(c))&&rutaConfirmada(o)&&o.rutaConf.origen==='persona'&&/ruta en lote/.test(o.rutaConf.nota)&&!!o.rutaFirma&&((f.mas.length||f.menos.length)?(o.rutaEditada.slice(-1)[0].etapa==='lote'&&o.rutaEditada.slice(-1)[0].mas.join()===f.mas.join()):true)}),JSON.stringify({n2,ej:plan.filas[0]&&{antes:plan.filas[0].antes,despues:plan.filas[0].despues,real:pasosProCompleta(plan.filas[0].o)}}));
    __check('RLT: el paso agregado toma el tiempo como la ficha (tiempoPaso: hoja LMO de la categoría, puntadas para bordado, el centro para lavado/plancha; 0 con aviso si no hay) y los pasos que ya tenía conservan su tiempo',plan.filas.every(f=>f.mas.every(c=>{const p=(f.o.ruta||[]).find(x=>x.centro===c);const t=+((tiempoPaso({centro:c,t:0},K(f.o.cat),samPorCentro(K(f.o.cat))||{},+f.o.puntadas||0)||{}).t)||0;return p&&p.t===t})),JSON.stringify(plan.sinT));
    __check('RLT: una sola línea de bitácora para el lote y una entrada de auditoría por orden',S.bitacora.length===nb+1&&/Ruta en lote · /.test(S.bitacora[S.bitacora.length-1].t)&&(S.params.auditoriaCambios||[]).length===nAud+ords.length&&(S.params.auditoriaCambios||[]).slice(-ords.length).every(a=>a.tipo==='ruta'&&/confirmada \(lote\)/.test(a.despues)));
    __check('RLT: tras aplicar, la ruta pendiente y la completa quedan coherentes (la pendiente dentro de la completa; la completa sin lo que salió)',plan.filas.every(f=>{const o=f.o;if(!o.rutaCompleta||!o.rutaCompleta.length)return true;const comp=new Set(o.rutaCompleta.map(p=>p.centro));return (o.ruta||[]).every(p=>comp.has(p.centro))&&!f.menos.some(c=>comp.has(c))&&!rutasDivergen(o)}));
    __check('RLT: salen de «por confirmar» (de la lista y de rutasPorDefinir); las órdenes no marcadas no se tocaron',ords.every(o=>!matchEdicionOrd(o,'porEditar')&&!rutasPorDefinir().includes(o)&&!rutasLista().includes(o))&&ordsFuera.every(([id,j])=>{const o=S.ordenes.find(x=>x.id===id);return !!o&&JSON.stringify([o.ruta,o.rutaCompleta,o.rutaConf,o.rutaEditada])===j}));
    __check('RLT: tras aplicar, el estado del lote queda limpio',!RLOTE.ids.length&&RLOTE.cens===null&&!RLOTE.tocado&&RLOTE.motivo==='');
    /* «Confirmadas» las muestra; marcarlas y volver a definir las pisa con aviso en el plan */
    RUT.ver='conf';render();__check('RLT: en «Confirmadas» aparecen las que acaban de confirmarse y, marcadas, el plan avisa que se pisan',ords.every(o=>rutasLista().includes(o))&&(ords.forEach(o=>RUT.sel.add(o.id)),abrirRutaLote(),planRutaLote(ordenesLote(),RLOTE.cens).confirmadas===ords.length&&/se pisan/.test(document.getElementById('modal').innerHTML)));cancelarRutaLote();RUT.sel=new Set();RUT.ver='pend';
    /* «Confirmar como están»: la ruta no cambia, solo se confirma */
    {const otras=rutasLista().filter(o=>(o.ruta||[]).length).slice(0,3);if(otras.length){const rutas0=otras.map(o=>JSON.stringify([o.ruta,o.rutaCompleta]));otras.forEach(o=>RUT.sel.add(o.id));const nb2=S.bitacora.length;const cf2=window.confirm;window.confirm=()=>true;const nc=confirmarRutasSel();window.confirm=cf2;
      __check('RLT: «Confirmar como están» confirma la ruta actual de las marcadas sin cambiar ningún paso, con auditoría por orden, una bitácora y la selección limpia',nc===otras.length&&otras.every((o,i)=>rutaConfirmada(o)&&/como está/.test(o.rutaConf.nota)&&JSON.stringify([o.ruta,o.rutaCompleta])===rutas0[i])&&S.bitacora.length===nb2+1&&RUT.sel.size===0);}else __check('RLT: hay órdenes por confirmar para probar «confirmar como están»',false);}
    /* un paso hecho se conserva aunque se desmarque (fase y cierre en piso) */
    const oH=ords.find(o=>pasosProCompleta(o).includes('corte'))||ords[0];const filaConf=faseMapeo().find(r=>normFase(r.fase)===normFase('7Confección'))||faseMapeo().find(r=>r.sistema==='confección');
    if(filaConf&&pasosProCompleta(oH).includes('corte')){const bakH=JSON.stringify(oH);aplicarFaseSistema(oH,filaConf.fase);RLOTE=RLOTE0();RLOTE.ids=[oH.id];RLOTE.cens=null;mRutaLote();
     const hechos=faseEstado(oH.fase,oH).hechos||[];const p2=planRutaLote([oH],(RLOTE.cens||[]).filter(c=>c!=='corte'));
     __check('RLT: un paso ya hecho según la fase (corte en 7Confección) se conserva aunque se desmarque, y el plan lo dice',hechos.includes('corte')&&p2.filas[0].conserv.includes('corte')&&!p2.filas[0].menos.includes('corte')&&p2.filas[0].despues.includes('corte'),JSON.stringify({hechos,conserv:p2.filas[0].conserv}));
     Object.assign(oH,JSON.parse(bakH));cerrar();}else __check('RLT: hay una orden con corte en la ruta para probar la conservación de pasos hechos',false,JSON.stringify({filaConf:!!filaConf,ruta:pasosProCompleta(oH)}));
    {const oC=ords.find(o=>pasosProCompleta(o).includes('corte')&&!(faseEstado(o.fase,o).hechos||[]).includes('corte'));if(oC){const bakA=S.avance[oC.id];S.avance[oC.id]=Object.assign({},bakA||{},{cierres:{corte:{pz:oC.cant,cant:oC.cant,faltan:0,u:'x',ts:new Date().toISOString()}}});
      const p3=planRutaLote([oC],pasosProCompleta(oC).filter(c=>c!=='corte'));__check('RLT: un paso cerrado en piso también se conserva aunque se desmarque',p3.filas[0].conserv.includes('corte')&&!p3.filas[0].menos.includes('corte'),JSON.stringify(p3.filas[0].conserv));if(bakA===undefined){S.avance[oC.id]={}}else S.avance[oC.id]=bakA;}else __check('RLT: hay una orden con corte pendiente para probar el cierre en piso',false);}
    __check('RLT: cancelar deja el estado del lote limpio',(RLOTE.ids=['x'],RLOTE.tocado=true,cancelarRutaLote(),!RLOTE.ids.length&&RLOTE.cens===null&&!RLOTE.tocado));
    cerrar();RLOTE=RLOTE0();RUT.sel=new Set();RUT.ver='pend';ORDF.tab='ord';PLAN=null;PLAN_ALL=null;}
   /* UX3 · tres cosas de la usuaria (20-sep): desplegar un grupo no manda la lista al inicio; filtros en una fila con fases; la ruta muestra lo hecho y la OT manda */
   {PERFIL=adminP0();RUT.sel=new Set();RUT.ver='pend';RUT.q='';RUT.fases=null;delete BUSQ['RUT.q'];GRP={};grpSt('rut').niveles=['fam','hija'];page='ordenes';ORDF.tab='rutas';render();
    const pg=()=>document.getElementById('p-ordenes');const sc=()=>pg().querySelector('[data-grp-scroll="rut"]');
    __check('UX3: la fila de filtros de Rutas trae buscador, agrupador en línea, filtro de fases y los chips, todo en una sola fila',!!sc()&&/togFaseRUT|RUT\.fases/.test(pg().innerHTML)&&pg().querySelectorAll('.filtros-blk select').length>=2&&!!pg().querySelector('.filtros-blk details.ffases'));
    /* desplegar el grupo más grande para que la lista tenga scroll; luego desplegar otro sin perder la posición */
    {const host=document.createElement('div');host.innerHTML='<div data-grp-scroll="zz" style="height:100px;overflow:auto"><div style="height:900px">x</div></div>';document.body.appendChild(host);host.firstChild.scrollTop=140;
     conScrollGRP('zz',()=>{host.innerHTML='<div data-grp-scroll="zz" style="height:100px;overflow:auto"><div style="height:900px">y</div></div>'});
     __check('UX3: redibujar la lista con conScrollGRP conserva el scroll del recuadro (el que se perdía al desplegar un grupo) y togGRP / marcar grupo pasan por ahí',host.firstChild.scrollTop===140&&/conScrollGRP\(id,render\)/.test(String(togGRP))&&/conScrollGRP\('rut',render\)/.test(String(selGrupoRut))&&!!sc(),JSON.stringify({top:host.firstChild.scrollTop}));host.remove();}
    /* filtro de fases como en las demás pantallas */
    {const fase=(rutasBase().find(o=>o.fase)||{}).fase;RUT.fases=new Set([fase]);render();
     __check('UX3: el filtro de fases acota la lista de Rutas',!!fase&&rutasLista().length>0&&rutasLista().every(o=>o.fase===fase)&&pg().querySelectorAll('tr.grp-row').length>0,JSON.stringify({fase,n:rutasLista().length}));RUT.fases=null;render();}
    /* cada fase dice cuántas órdenes y prendas tiene (usuaria, 20-sep: «las fases y en paréntesis cuántas unidades») */
    {RUT.fases=null;RUT.ver='pend';RUT.q='';render();const nomDe=l=>l.textContent.trim().replace(/\s*\([^()]*\)$/,'');const lab=(f)=>[...pg().querySelectorAll('.filtros-blk details.ffases label')].find(l=>nomDe(l)===f);
     const pend=rutasBase().filter(o=>!rutaConfirmada(o));const f0=pend.length?(pend[0].fase||'Sin fase'):null;const enF=pend.filter(o=>(o.fase||'Sin fase')===f0);const nF=enF.length,pzF=enF.reduce((a,o)=>a+(+o.cant||0),0);
     __check('UX3: el filtro de fases de Rutas dice por cada fase cuántas órdenes y prendas hay (con el chip puesto: por confirmar) y coincide con la lista',!!f0&&!!lab(f0)&&lab(f0).textContent.includes('('+num(nF)+' órdenes · '+num(pzF)+' prendas)')&&rutasLista().filter(o=>(o.fase||'Sin fase')===f0).length===nF,JSON.stringify({f0,nF,pzF,txt:f0&&lab(f0)&&lab(f0).textContent.trim()}));
     RUT.ver='conf';render();const conf=rutasBase().filter(o=>rutaConfirmada(o));const nC=conf.filter(o=>(o.fase||'Sin fase')===f0).length;const pzC=conf.filter(o=>(o.fase||'Sin fase')===f0).reduce((a,o)=>a+(+o.cant||0),0);
     __check('UX3: los conteos del filtro siguen al chip (Confirmadas cuenta otras órdenes; una fase sin órdenes dice 0) y no se vacían al desmarcar fases',!!f0&&!!lab(f0)&&lab(f0).textContent.includes(nC?'('+num(nC)+' órdenes · '+num(pzC)+' prendas)':'(0)')&&(()=>{RUT.fases=new Set([FASE_NINGUNA]);render();const ok=!!lab(f0)&&lab(f0).textContent.includes(nC?'('+num(nC)+' órdenes':'(0)');RUT.fases=null;return ok})(),JSON.stringify({f0,nC,pzC,txt:f0&&lab(f0)&&lab(f0).textContent.trim()}));RUT.ver='pend';render();}
    /* la ruta por orden marca los pasos ya hechos (están en la completa, no en la pendiente) */
    {const o=S.ordenes.find(x=>abierta(x)&&lanzada(x)&&!rutaNoAplica(x)&&(x.rutaCompleta||[]).some(p=>p.centro==='corte')&&(x.rutaCompleta||[]).some(p=>p.centro==='empaque'));const filaConf=faseMapeo().find(r=>normFase(r.fase)===normFase('7Confección'));
     if(o&&filaConf){const bak=JSON.stringify(o);aplicarFaseSistema(o,filaConf.fase);const pend=(o.ruta||[]).map(p=>p.centro);mRutaCentro(o.id);const cb=document.getElementById('rc-corte');const h=document.getElementById('modal').innerHTML;
      __check('UX3: en fase 7Confección el corte ya no está en la ruta pendiente pero la ficha de ruta lo muestra MARCADO y «hecho» (antes salía desmarcado)',!pend.includes('corte')&&!!cb&&cb.checked&&cb.disabled&&/hecho/.test(h),JSON.stringify({pend,checked:cb&&cb.checked}));
      /* lo que dice Odoo se compara contra la ruta completa */
      const rc=pasosProCompleta(o);o.ot={};rc.forEach(c=>{o.ot[c]={estado:c==='corte'?'terminado':'para hacer',odoo:'WC-'+c}});delete o.rutaConf;PLAN=null;
      const d=diagRutaOdoo(o,new Set());__check('UX3: con OT en cada paso de la ruta completa (corte terminado), Odoo «coincide exacto» aunque el corte ya no esté en la pendiente (antes salía «Odoo tiene Corte, la ruta no»)',d.estado==='coincide',JSON.stringify(d));
      cerrar();Object.assign(o,JSON.parse(bak));if(!JSON.parse(bak).ot)delete o.ot;PLAN=null;PLAN_ALL=null;}else __check('UX3: hay una orden con corte y empaque en la ruta completa para probar',false);}
    /* la OT manda sobre la ruta: un centro con orden de trabajo entra a la ruta (hecho → solo en la completa; pendiente → en las dos); nunca quita */
    {const o=S.ordenes.find(x=>abierta(x)&&lanzada(x)&&!rutaNoAplica(x)&&!rutaConfirmada(x)&&(x.rutaCompleta||[]).length&&!pasosProCompleta(x).includes('bordado')&&!pasosProCompleta(x).includes('botones')&&CE('bordado')&&CE('botones'));
     if(o){const bak=JSON.stringify(o);const antes=pasosProCompleta(o).slice();o.ot=Object.assign({},o.ot||{},{bordado:{estado:'terminado',odoo:'WC-B'},botones:{estado:'para hacer',odoo:'WC-BT'}});
      const add=empatarRutaConOT(o);const comp=o.rutaCompleta.map(p=>p.centro);const pend=(o.ruta||[]).map(p=>p.centro);
      __check('UX3: empatarRutaConOT agrega a la ruta los centros con OT: el terminado (bordado) solo a la completa, el pendiente (botones) a la completa y a la pendiente, en su orden de paso, marcados ot y en o.rutaOT; no quita nada',add.join()==='bordado,botones'&&comp.includes('bordado')&&comp.includes('botones')&&!pend.includes('bordado')&&pend.includes('botones')&&antes.every(c=>comp.includes(c))&&o.rutaCompleta.find(p=>p.centro==='bordado').ot===true&&add.every(c=>(o.rutaOT||[]).includes(c))&&comp.indexOf('botones')>comp.indexOf('modulos')&&comp.indexOf('botones')<comp.indexOf('empaque'),JSON.stringify({add,comp,pend}));
      __check('UX3: volver a empatar no agrega nada (idempotente)',empatarRutaConOT(o).length===0&&o.rutaCompleta.filter(p=>p.centro==='bordado').length===1);
   ...
   /* FS20 · orden de las fases según el archivo de la usuaria (20-sep): decimales, mismo número = paralelas; las listas se ordenan por esa secuencia */
   {delete S.params.fasesSecuencia19;sembrarFasesSecuencia19();delete S.params.fasesSecuencia20;sembrarFasesSecuencia20();   /* las pruebas anteriores vacían y re-siembran la tabla 1: se corre la secuencia completa (19 y luego 20) como en el arranque */
    __check('FS20: la siembra del 20-sep corrió (bandera) y dejó la secuencia del archivo: Servicios y Terminados 8 < Empaque 8.1 < Empaque Terminado 8.2 … Cotizaciones 8.6; Facturado / Stand by al final',!!S.params.fasesSecuencia20&&FASES_SEC_20.every(([f,n])=>!filaFaseDe(f)||secuenciaDe(f)===n)&&secuenciaDe('8Servicios y Terminados')===8&&secuenciaDe('8Empaque')===8.1&&secuenciaDe('Facturado')===9,JSON.stringify(FASES_SEC_20.filter(([f,n])=>filaFaseDe(f)&&secuenciaDe(f)!==n).slice(0,5)));
    __check('FS20: con ese orden, volver de Empaque a Servicios y Terminados es devolución (pide motivo); Lavandería ↔ Botones ↔ Servicios y Terminados son paralelas; Confección → Bordado ahora sí es devolución; Embodegado ↔ Exportación ↔ Cotizaciones paralelas',esDevolucionFase('8Empaque','8Servicios y Terminados')&&!esDevolucionFase('8Lavanderia','8Botones')&&!esDevolucionFase('8Botones','8Servicios y Terminados')&&esDevolucionFase('7Confección','6Bordado')&&!esDevolucionFase('8Exportacion','8Cotizaciones')&&!esDevolucionFase('8Servicios y Terminados','8Empaque'));
    const seq=JSON.stringify(faseMapeo().map(r=>[r.fase,r.secuencia]));sembrarFasesSecuencia20();__check('FS20: la siembra es de una sola vez',JSON.stringify(faseMapeo().map(r=>[r.fase,r.secuencia]))===seq);
    __check('FS20: cmpFases ordena por la secuencia de la tabla 1 (Servicios y Terminados antes que Empaque, aunque alfabéticamente sea al revés) y cae al número de Odoo si falta',['8Empaque','8Servicios y Terminados','8Botones','7Pulido'].sort(cmpFases).join('|')==='7Pulido|8Botones|8Servicios y Terminados|8Empaque'&&cmpFases('2Planificacion','3Trazos')<0);
    /* en pantalla: la pestaña Rutas agrupada por fase muestra Servicios y Terminados antes que Empaque */
    {PERFIL=adminP0();RUT.sel=new Set();RUT.ver='todas';RUT.q='';RUT.fases=null;GRP={};grpSt('rut').sembrado=true;grpSt('rut').niveles=['fase'];page='ordenes';ORDF.tab='rutas';render();
     const cabs=[...document.querySelectorAll('#p-ordenes tr.grp-row')].map(tr=>tr.textContent.replace(/\s+/g,' '));const iS=cabs.findIndex(t=>/Servicios y Terminados/.test(t)),iE=cabs.findIndex(t=>/8Empaque\b/.test(t)&&!/Terminado/.test(t));
     __check('FS20: en Rutas agrupado por fase, «8Servicios y Terminados» va antes que «8Empaque» (antes salía Empaque primero por orden alfabético)',iS>=0&&iE>=0&&iS<iE,JSON.stringify({iS,iE,cabs:cabs.slice(0,3)}));
     grpSt('rut').niveles=['fam','hija'];RUT.ver='pend';ORDF.tab='ord';}}
   /* EX · excedentes (usuaria, 20-sep): prenda ya hecha, solo el paso final; ODC y prendas en Rutas; la OT no toca una ruta confirmada */
   {PERFIL=adminP0();delete S.params.palabrasExcedente;delete S.params.centroExcedente;
    __check('EX: los parámetros nacen visibles y editables (palabra EXCEDENTE, centro Empaque) y esExcedente lee la ODC / proyecto o la marca a mano',palabrasExcedente().join()==='excedente'&&centroExcedente()==='empaque'&&esExcedente({odc:'2784-EXCEDENTES LISTOS'})&&!esExcedente({odc:'3033'})&&esExcedente({odc:'3033',excedente:true})&&!esExcedente({odc:'2784-EXCEDENTES',excedente:false}));
    page='config';CONF.tab='cal';render();__check('EX: Calendario y parámetros muestra los dos parámetros con su explicación',/setPalabrasExcedente/.test(document.getElementById('p-config').innerHTML)&&/setCentroExcedente/.test(document.getElementById('p-config').innerHTML));
    /* al cargar: una tarea nueva con ODC «EXCEDENTES» nace con la ruta de solo el paso final, sin textil, marcada estimada */
    {const H=tareaRows[0];const col=n=>H.indexOf(n);const kh=S.categorias.find(k=>k.padre&&tieneHojaLMO(k)&&(samPorCentro(k)||{}).empaque>0)||S.categorias.find(k=>k.padre);const kp=K(kh.padre);
     const f=new Array(H.length).fill(null);f[col('Orden de producción')]='WH/MO/99401';f[col('Proyecto')]='NOVIEMBRE 2026';f[col('Cliente')]='PRICE CLUB';f[col('ODC')]='2784-EXCEDENTES LISTOS';f[col('Stilo')]='EX1';f[col('Categoría Padre')]=kp.n;f[col('Categoría Hija')]=kh.n;f[col('Color')]='BIRCH';f[col('Fase')]='2Planificacion';f[col('Estado OP')]='confirmed';f[col('Pedido')]=11;f[col('Fecha Entrega')]='2026-12-01';
     const p=planTarea([tareaRows[0],f,...tareaRows.slice(1,300)],'TAREA_EX.xlsx');const o=p.ordenes.find(x=>x.op==='WH/MO/99401');
     __check('EX: una orden nueva con «EXCEDENTES» en la ODC nace excedente: ruta = solo Empaque (sin tejeduría, tintorería ni corte), estimada con nota, y la lista la marca',!!o&&o.excedente===true&&(o.rutaCompleta||[]).map(x=>x.centro).join()==='empaque'&&(o.ruta||[]).every(x=>x.centro==='empaque')&&o.rutaConf&&o.rutaConf.estado==='estimada'&&/excedente/.test(o.rutaConf.nota)&&!o.rutaDefecto&&/excedente</.test(tagExcedente(o)),JSON.stringify(o&&{ruta:(o.rutaCompleta||[]).map(x=>x.centro),conf:o.rutaConf}));
     __check('EX: las demás no llevan la marca de excedente ni la ruta de solo paso final por contagio',p.ordenes.filter(x=>x.op!=='WH/MO/99401'&&!esExcedente(x)).every(x=>!x.excedente&&!(x.rutaConf&&/excedente/.test(x.rutaConf.nota||''))));}
    /* Rutas: columnas ODC y prendas por fila */
    {RUT.sel=new Set();RUT.ver='pend';RUT.q='';RUT.fases=null;GRP={};grpSt('rut').sembrado=true;grpSt('rut').niveles=[];page='ordenes';ORDF.tab='rutas';render();const h=document.getElementById('p-ordenes').innerHTML;const o=rutasLista()[0];
     __check('EX: la lista de Rutas trae ODC y prendas por orden (no solo el total del grupo)',/<th>ODC<\/th><th class="num">Prendas<\/th>/.test(h)&&!!o&&new RegExp('<td class="num">'+num(o.cant)+'</td>').test(h),o&&o.op);grpSt('rut').niveles=['fam','hija'];ORDF.tab='ord';}
    /* la OT no toca una ruta confirmada */
    {const o=S.ordenes.find(x=>abierta(x)&&lanzada(x)&&!rutaNoAplica(x)&&(x.rutaCompleta||[]).length&&!pasosProCompleta(x).includes('bordado')&&CE('bordado'));
     if(o){const bak=JSON.stringify(o);o.rutaConf={estado:'confirmada',origen:'persona',u:'x',ts:new Date().toISOString(),nota:'prueba'};o.ot=Object.assign({},o.ot||{},{bordado:{estado:'terminado',odoo:'WC-B'}});
      const add=empatarRutaConOT(o);__check('EX: con la ruta confirmada por una persona, la OT no agrega nada (bordado con OT queda fuera de la ruta y se reporta, no se aplica)',add.length===0&&!pasosProCompleta(o).includes('bordado'));
      delete o.rutaConf;const add2=empatarRutaConOT(o);__check('EX: sin confirmar, la OT sí agrega el paso',add2.join()==='bordado'&&pasosProCompleta(o).includes('bordado'));
      Object.assign(o,JSON.parse(bak));if(!JSON.parse(bak).ot)delete o.ot;if(!JSON.parse(bak).rutaOT)delete o.rutaOT;if(!JSON.parse(bak).rutaConf)delete o.rutaConf;PLAN=null;PLAN_ALL=null;}else __check('EX: hay una orden sin bordado para probar la OT',false);
     __check('EX: aplicarOT reporta las confirmadas que la OT no tocó (registro de cargas y Reporte OT)',/confNoTocadas/.test(String(aplicarOT))&&/confNoTocadas/.test(String(mReporteOT))&&/confNoTocadas/.test(String(resumenCargaTxt)));}
    delete S.params.palabrasExcedente;delete S.params.centroExcedente;}
   /* PN · Planificar el mes en dos pasos (usuaria, 20-sep): 1 Nivelación (días, personas por centro, ¿alcanza?) → 2 Plan mensual; el escenario confirmado manda en todo el sistema */
   {PERFIL=adminP0();const ym=hoy().slice(0,7);PM.mes=ym;PM.paso=1;NIVUI.area='corte';NIVUI.esc={};SIM={on:false,ym:null,rec:null,cambios:{},motivo:''};const bakAj=JSON.stringify(S.params.ajustesCap||null);page='plan';render();const pg=()=>document.getElementById('p-plan');let h=pg().innerHTML;
    ...
    {const rc=S.recursos.find(r=>r.activa&&r.centro==='corte'&&r.id!=='maquila');if(rc){const sem=semanasMes(ym);const dia=sem.map(w=>w.dias.find(d=>labR(d,rc))).find(Boolean);const cap0=dia?capDia(rc,dia):null;const min0=+(rc.min||0);
      ...
      nivCapSet(ym,rc.id,'min','9');const okSim=sem.every(w=>SIM.cambios[w.ini]&&SIM.cambios[w.ini][rc.id]&&SIM.cambios[w.ini][rc.id].min===540);const cap1=dia?capDia(rc,dia):null;
      __check('HP: escribir 9 horas guarda 540 min/día en el escenario de TODAS las semanas y la capacidad del día sube en proporción (9/8)',okSim&&cap0!=null&&min0>0&&Math.abs(cap1-cap0*540/min0)<1e-6,JSON.stringify({cap0,cap1,min0}));
      const bak=JSON.stringify(S.params.ajustesCap||null);const el=document.getElementById('sim-motivo')||document.body.appendChild(Object.assign(document.createElement('input'),{id:'sim-motivo'}));el.value='hora extra (prueba HP)';const nb=S.bitacora.length;guardarAjustesCap(ym);
      const g=((S.params.ajustesCap||{})[ym]||{}).semanas||{};const okG=sem.every(w=>g[w.ini]&&g[w.ini][rc.id]&&g[w.ini][rc.id].min===540&&g[w.ini][rc.id].motivo==='hora extra (prueba HP)');
      __check('HP: confirmar el escenario graba las horas como min/día del ajuste de cada semana, con motivo y bitácora, y capDia lo usa sin escenario abierto',okG&&!SIM.on&&Math.abs(capDia(rc,dia)-cap0*540/min0)<1e-6&&S.bitacora.length>nb&&/min\/día 480 → 540|min\/día \d+ → 540/.test(S.bitacora[S.bitacora.length-1].t),JSON.stringify({okG,ult:S.bitacora[S.bitacora.length-1].t.slice(0,120)}));
      const b=JSON.parse(bak);if(b)S.params.ajustesCap=b;else delete S.params.ajustesCap;CAPM=null;PLAN=null;PLAN_ALL=null;}}
    /* MQ · maquila por descarte (usuaria, 20-sep noche: «maquila puede hacer todo; llenamos primero nuestros módulos y con lo que falta, la maquila») */
    {const bakMO=JSON.stringify(S.params.maquilaOrden||null),bakMM=JSON.stringify(S.params.maquilaOrdenMeta||null),bakPD=S.params.maquilaPorDescarte;delete S.params.maquilaOrden;delete S.params.maquilaOrdenMeta;delete S.params.maquilaPorDescarte;
     const l=maquilaOrden();const fams=familiasPadre().map(k=>k.id);
     __check('MQ: el orden de descarte a maquila se siembra una vez desde la polivalencia (una fila por familia, sugerido, menor polivalencia primero) y es lo configurado',l.length===fams.length&&fams.every(f=>l.some(x=>x.fam===f))&&l.every(x=>x.sugerido)&&l.every((x,i)=>i===0||(l[i-1].poli==null?101:l[i-1].poli)<=(x.poli==null?101:x.poli))&&!!S.params.maquilaOrdenMeta&&maquilaPorDescarte(),JSON.stringify(l.slice(0,4).map(x=>[(K(x.fam)||{}).n,x.poli])));
     const f0=l[0].fam,f1=l[1]&&l[1].fam;moverMaquilaOrden(f0,1);const l2=maquilaOrden();__check('MQ: mover una familia cambia el orden, deja de ser sugerido y queda en bitácora; «no mandar» y confirmar se guardan',!!f1&&l2[0].fam===f1&&l2[1].fam===f0&&!l2[1].sugerido&&/Orden de descarte a maquila/.test(S.bitacora[S.bitacora.length-1].t)&&(()=>{setMaquilaNoMandar(f0,true);confirmarMaquilaOrden();return maquilaOrden().find(x=>x.fam===f0).noMandar===true&&maquilaOrden().every(x=>!x.sugerido)&&S.params.maquilaOrdenMeta.confirmado===true})());setMaquilaNoMandar(f0,false);
     page='config';CONF.tab='nivel';render();__check('MQ: Configuración → Nivelación muestra la tabla «Qué mandamos primero a maquila» con la casilla del descarte',/Qué mandamos primero a maquila/.test(document.getElementById('p-config').innerHTML)&&/setMaquilaPorDescarte/.test(document.getElementById('p-config').innerHTML));
     /* sin capacidad en planta (escenario: 0 personas en todos los módulos) todo el saldo de Confección hay que mandarlo */
     page='plan';PM.paso=1;NIVUI.area='maquila';NIVUI.meses=nivUIMesesDisp().filter(m=>m<=ym);const mods=S.recursos.filter(r=>r.activa&&r.centro==='modulos'&&r.id!=='maquila');mods.forEach(m=>nivCapSet(ym,m.id,'pers','0'));NIVC=null;
     const d=maquilaDescarte();const sal=saldoProceso('modulos',NIVUI.meses,null);const conSAM=sal.ordenes.filter(o=>samOrdenCentro(o,'modulos')!=null&&pendCentroUnid(o,'modulos')>0);
     const ordenCfg=maquilaOrden().filter(x=>!x.noMandar).map(x=>x.fam);const famsD=d.porFam.map(g=>g.fam);const enOrden=famsD.every((f,i)=>i===0||ordenCfg.indexOf(famsD[i-1])<=ordenCfg.indexOf(f)||ordenCfg.indexOf(f)<0);
     __check('MQ: sin capacidad en planta, «hay que mandar» = todo el saldo con SAM de Confección, en órdenes enteras, repartido por familia en el orden configurado',d.estado!=='faltante'&&d.deficitMin>0&&d.totalU>0&&Math.abs(d.totalU-conSAM.reduce((a,o)=>a+pendCentroUnid(o,'modulos'),0))<1&&enOrden&&d.ordenes.every(x=>Number.isInteger(Math.round(x.u))),JSON.stringify({totalU:d.totalU,saldoU:conSAM.reduce((a,o)=>a+pendCentroUnid(o,'modulos'),0),fams:famsD.map(f=>(K(f)||{}).n),deficitMin:d.deficitMin,estado:d.estado}));
     render();const pg2=document.getElementById('p-plan');const card=pg2.querySelector('.niv-card[data-area=maquila]');
     __check('MQ: el recuadro Maquila dice cuántas unidades hay que mandar (pastilla ámbar «enviar N u») y al elegirlo se ve el detalle por familia con las órdenes propuestas y el botón de mandar',!!card&&/enviar/.test(card.textContent)&&card.classList.contains('est-riesgo')&&/Maquila = lo que no cabe/.test(pg2.innerHTML)&&pg2.querySelectorAll('input[data-maq-sel]').length===d.ordenes.length&&/Mandar las marcadas a maquila/.test(pg2.innerHTML),JSON.stringify({card:card&&card.textContent.replace(/\s+/g,' ').slice(0,80),n:pg2.querySelectorAll('input[data-maq-sel]').length}));
     /* «no mandar» una familia con saldo la deja fuera y avisa lo que no cabe */
     const fSal=famsD[0];setMaquilaNoMandar(fSal,true);NIVC=null;const d2=maquilaDescarte();__check('MQ: una familia marcada «no mandar» no entra al descarte y el detalle avisa las horas que quedan sin cubrir',!d2.porFam.some(g=>g.fam===fSal)&&d2.faltaSinCubrir>0&&d2.noCabe.includes(fSal),JSON.stringify({fSal:(K(fSal)||{}).n,falta:d2.faltaSinCubrir}));setMaquilaNoMandar(fSal,false);NIVC=null;
     /* mandar una orden a maquila: recurso fijo, auditoría, sale del saldo de planta, el motor la manda a la maquila; devolverla la regresa */
     const o1=d.ordenes[0]&&d.ordenes[0].o;if(o1){const cf=window.confirm;window.confirm=()=>true;const nAud=(S.params.auditoriaCambios||[]).length;const nMarc=marcarMaquila([o1.id],'prueba MQ');
       const sal2=saldoProceso('modulos',NIVUI.meses,null);const P=programar();const enMaq=(P.pro||[]).filter(e=>e.op===o1.op&&e.centro==='modulos');const otrosMaq=(P.pro||[]).filter(e=>e.centro==='modulos'&&e.rec==='maquila'&&!S.ordenes.some(o=>o.op===e.op&&(o.recursoFijo||{}).modulos==='maquila'));
       __check('MQ: mandar una orden a maquila la deja con recurso fijo maquila (auditoría tipo maquila, bitácora), la saca del saldo de planta y el programa la pone en la maquila; ninguna orden sin marca cae en la maquila (maquila por descarte)',nMarc===1&&(o1.recursoFijo||{}).modulos==='maquila'&&(S.params.auditoriaCambios||[]).length===nAud+1&&(S.params.auditoriaCambios||[]).slice(-1)[0].tipo==='maquila'&&!sal2.ordenes.some(o=>o.id===o1.id)&&sal2.maquila>=pendCentroUnid(o1,'modulos')-1&&(!enMaq.length||enMaq.every(e=>e.rec==='maquila'))&&!otrosMaq.length,JSON.stringify({nMarc,enMaq:enMaq.length,otrosMaq:otrosMaq.length,recs:[...new Set(enMaq.map(e=>e.rec))]}));
       const ok2=quitarMaquila(o1.id,'devolver (prueba MQ)');__check('MQ: devolver a planta quita el recurso fijo (con motivo, confirmación y auditoría) y la orden vuelve al saldo',ok2&&!(o1.recursoFijo||{}).modulos&&saldoProceso('modulos',NIVUI.meses,null).ordenes.some(o=>o.id===o1.id));
       __check('MQ: quitarMaquila y marcarMaquila piden confirmación (GUARDIA) y solo con permiso «programa»',/confirm\(/.test(String(quitarMaquila))&&/confirm\(/.test(String(marcarMaquila))&&/puede\('programa'\)/.test(String(marcarMaquila)));window.confirm=cf;}
     /* revisión adversarial (21-sep): marcar X minutos baja el déficit exactamente X (lo marcado no se cuenta dos veces como capacidad) */
     {const cf=window.confirm;window.confirm=()=>true;const d0=maquilaDescarte();const oX=d0.ordenes.find(x=>x.o&&(x.o.recursoFijo||{}).modulos!=='maquila'&&!((x.o.progCentro||{}).modulos||{}).rec);
      if(oX){const defA=d0.deficitMin;marcarMaquila([oX.o.id],'prueba doble conteo');NIVC=null;const r1=nivUICalcular('modulos');const d1=maquilaDescarte();
       __check('MQ: mandar una orden de X minutos baja el déficit de Confección en X (no en 2X): lo marcado sale del saldo y NO vuelve a sumar como capacidad',Math.abs((defA-r1.deficitMin)-oX.min)<Math.max(1,oX.min*0.02)&&d1.yaU>=oX.u-1,JSON.stringify({defA,def1:r1.deficitMin,min:oX.min,yaU:d1.yaU}));
       quitarMaquila(oX.o.id,'devolver (prueba)');NIVC=null;}
      /* el recurso fijado en la cola (progCentro.rec) manda igual que en el motor: mandar a maquila lo suelta; devolver lo limpia */
      const oY=(maquilaDescarte().ordenes[0]||{}).o;if(oY){oY.progCentro=Object.assign({},oY.progCentro||{},{modulos:Object.assign({},(oY.progCentro||{}).modulos||{},{rec:'mod1'})});const antesVa=vaAMaquila(oY);marcarMaquila([oY.o?oY.o.id:oY.id],'prueba pc.rec');
       __check('MQ: si el supervisor había fijado un módulo en la cola, mandar a maquila suelta ese módulo (si no, el motor la dejaría ahí) y vaAMaquila/saldoProceso/motor leen la MISMA precedencia',!antesVa&&vaAMaquila(oY)&&!((oY.progCentro||{}).modulos||{}).rec&&recFijadoDe(oY,'modulos')==='maquila'&&/recFijadoDe\(o,p\.centro\)/.test(String(programar))&&/vaAMaquila\(o\)/.test(String(saldoProceso)));
       quitarMaquila(oY.id,'devolver');delete oY.progCentro.modulos;}
      /* todas las familias con saldo marcadas «no mandar» = déficit a la vista, nunca «no hace falta» */
      {const bakL=JSON.stringify(maquilaOrden());maquilaOrden().forEach(x=>{x.noMandar=true});NIVC=null;const d2=maquilaDescarte();render();const card=pg().querySelector('.niv-card[data-area=maquila]');
       __check('MQ: con todas las familias en «no mandar» y déficit, el recuadro dice «✕ no cabe y no se manda» (nunca «✓ no hace falta») y el detalle lo explica',d2.totalU===0&&d2.faltaSinCubrir>0&&!!card&&card.classList.contains('est-deficit')&&/no cabe y no se manda/.test(card.textContent)&&!/no hace falta/.test(card.textContent)&&(()=>{NIVUI.area='maquila';render();const ok=/no cabe y no se manda/.test(pg().innerHTML)&&!/la planta alcanza/.test(pg().innerHTML);NIVUI.area='maquila';return ok})(),JSON.stringify({totalU:d2.totalU,falta:d2.faltaSinCubrir,card:card&&card.textContent.replace(/\s+/g,' ').slice(0,80)}));
       S.params.maquilaOrden=JSON.parse(bakL);NIVC=null;}
      /* el criterio de qué orden va primero es un parámetro visible (Configuración → Nivelación) */
      {const a=maquilaCriterio();setMaquilaCriterio('cercana');NIVC=null;const dc=maquilaDescarte();const g=dc.porFam[0];const okOrd=!g||g.ordenes.every((x,i)=>i===0||String(fechaMetaDe(g.ordenes[i-1].o)||'9')<=String(fechaMetaDe(x.o)||'9'));
       __check('MQ: «qué orden va primero» es un parámetro (lejana | cercana | mayor | menor) con selector y bitácora; con «cercana» las órdenes de cada familia salen de la más cercana a la más lejana y el detalle dice cuál rige y cuánto se pasa la última',maquilaCriterio()==='cercana'&&okOrd&&/criterio de qué orden va primero/.test(S.bitacora[S.bitacora.length-1].t)&&typeof dc.sobraMin==='number'&&/setMaquilaCriterio/.test(maquilaOrdenHTML()),JSON.stringify({crit:dc.criterio,sobra:dc.sobraMin}));setMaquilaCriterio(a);NIVC=null;}
      window.confirm=cf;}
     /* el interruptor nace explícito y con su efecto en bitácora (siembra de una vez) */
     {const b1=S.params.maquilaPorDescarte,b2=S.params.maquilaDescarteSembrado;delete S.params.maquilaPorDescarte;delete S.params.maquilaDescarteSembrado;const nb=S.bitacora.length;sembrarMaquilaPorDescarte();
      __check('MQ: la siembra deja el parámetro explícito en 1, una sola vez, y anota en bitácora cuántas órdenes que el motor mandaba solo a maquila vuelven a los módulos',S.params.maquilaPorDescarte===1&&!!S.params.maquilaDescarteSembrado&&S.bitacora.length===nb+1&&/Maquila por descarte ACTIVADO/.test(S.bitacora[nb].t)&&(sembrarMaquilaPorDescarte(),S.bitacora.length===nb+1),S.bitacora[nb]&&S.bitacora[nb].t.slice(0,160));
      S.params.maquilaPorDescarte=b1===undefined?1:b1;S.params.maquilaDescarteSembrado=b2||S.params.maquilaDescarteSembrado;PLAN=null;PLAN_ALL=null;}
     /* el tambor: polivalencia sin dato NO se pinta de verde; la capacidad es el promedio de la nivelación */
     {NIVUI.area='modulos';render();const hh=pg().innerHTML;const i=hh.indexOf('Qué lleva cada módulo');const seg=hh.slice(i);const m0=S.recursos.find(r=>r.activa&&r.centro==='modulos'&&r.id!=='maquila');const sinPoli=m0&&!Object.keys(m0.poli||{}).length;
      __check('TB: una celda sin dato de polivalencia no lleva fondo y lo dice; la capacidad del módulo sale del mismo promedio por día hábil que usa la nivelación (con escenario y ajustes)',/sin fondo = sin dato/.test(seg)&&(!sinPoli||/polivalencia: sin dato/.test(seg))&&/promedio por día hábil/.test(seg)&&/capRec\[m\.id\]/.test(String(tamborModulosHTML)),JSON.stringify({sinPoli}));NIVUI.area='corte';}
    ...
    {const cards=[...pg().querySelectorAll('.niv-card')];const grupos=[...pg().querySelectorAll('.niv-grupo')];const tit=grupos.map(g=>g.querySelector('.niv-grupo-t').textContent.trim());
     ...
     else __check('PN: (no hay en el volcado un centro con saldo solo sin SAM; la regla sinSAMtodo existe en nivUICalcular)',/sinSAMtodo/.test(String(nivUICalcular))&&/sinSAMtodo/.test(String(nivUIQueFalta)));
     /* un centro por días cuenta todas las unidades (no mide minutos) */
    ...
    if(r){const w=semanasMes(ym);const dia=w.flatMap(x=>x.dias).find(d=>labR(d,r));const capBase=capDia(r,dia);const nuevo=(r.pers||0)+3;
     nivPersSet(ym,r.id,nuevo);h=pg().innerHTML;const det=nivUICapacidad('corte').detalle.find(x=>x.rec===r.id);
     __check('PN: escribir personas en el paso 1 abre el escenario para TODAS las semanas del mes y la capacidad de la nivelación lo usa al instante (promedio por día hábil del mes, marcado «INCLUYE el escenario»), sin guardar nada',SIM.on&&SIM.ym===ym&&w.every(x=>SIM.cambios[x.ini]&&SIM.cambios[x.ini][r.id]&&SIM.cambios[x.ini][r.id].pers===nuevo)&&Math.abs(capDia(r,dia)-capBase*nuevo/(r.pers||1))<1e-6&&!!det&&Math.abs(det.cap-capDia(r,dia))<1e-6&&/INCLUYE el escenario/.test(nivUICapacidad('corte').nota)&&/escenario<\/span>/.test(h)&&/id="sim-motivo"/.test(h)&&JSON.stringify(S.params.ajustesCap||null)===bakAj,JSON.stringify({capBase,capAhora:capDia(r,dia),det:det&&det.cap}));
     const al=window.alert;const alerts=[];window.alert=m=>alerts.push(String(m));document.getElementById('sim-motivo').value='';guardarAjustesCap(ym);
     __check('PN: confirmar sin motivo no guarda nada',SIM.on&&alerts.some(a=>/motivo/i.test(a))&&JSON.stringify(S.params.ajustesCap||null)===bakAj);
     document.getElementById('sim-motivo').value='prueba PN: entran 3 a corte';const nb=S.bitacora.length;guardarAjustesCap(ym);window.alert=al;
     const aj=((S.params.ajustesCap||{})[ym]||{}).semanas||{};h=pg().innerHTML;
     __check('PN: «Confirmar escenario» graba las personas como ajuste de cada semana del mes (con motivo y bitácora), cierra el escenario y desde ahí capDia —el programa, el plan, la carga y la nivelación— usa ese valor; la fila dice «vigente»',!SIM.on&&w.every(x=>aj[x.ini]&&aj[x.ini][r.id]&&aj[x.ini][r.id].pers===nuevo&&/prueba PN/.test(aj[x.ini][r.id].motivo))&&Math.abs(capDia(r,dia)-capBase*nuevo/(r.pers||1))<1e-6&&Math.abs(nivUICapacidad('corte').detalle.find(x=>x.rec===r.id).cap-capDia(r,dia))<1e-6&&!/INCLUYE el escenario/.test(nivUICapacidad('corte').nota)&&/vigente<\/span>/.test(h)&&S.bitacora.length>nb&&(r.pers||0)!==nuevo,JSON.stringify({sem:Object.keys(aj).length,pers:r.pers}));
     __check('PN: la base de Configuración no cambió',(S.recursos.find(x=>x.id===r.id).pers||0)===(r.pers||0)&&(r.pers||0)!==nuevo);
   ...
   /* T21 · tiempos del 21-sep (decisión de la usuaria): lo que manda sobre la hoja por tipo (camisetas 4 y pico), corte/empaque de las sin hoja, botones estándar 0,40,
      minuto de cordones/apliques/sublimado; nada toca las órdenes cargadas hasta «Aplicar» (antes/después) */
   {PERFIL=adminP0();const cf=window.confirm;window.confirm=()=>true;const al=window.alert;const alerts=[];window.alert=m=>alerts.push(String(m));
    const bak={sem:S.params.tiempos21Sembrado,res:S.params.tiempos21Resumen,bot:S.params.botonesEstandar,cord:JSON.stringify(CE('cordones')||null),apl:JSON.stringify(CE('apliques')||null),sub:JSON.stringify(CE('sublimado')||null),ap:JSON.stringify(S.params.tiemposAplicados||null)};
    /* catálogo de prueba: CAMISETAS / Camiseta CR con la hoja de una categoría con operaciones; JOGGER / Jogger sin hoja */
    const conHoja=S.categorias.find(k=>k.padre&&tieneHojaLMO(k)&&(samPorCentro(k)||{}).modulos>0);
    const ya=nom=>S.categorias.find(x=>x.padre&&!x.unificadaEn&&normFase(x.n||'')===normFase(nom));const creadas=[];
    const padC=ya('Camiseta CR')?K(ya('Camiseta CR').padre):{id:uid(),n:'CAMISETAS'};const hC=ya('Camiseta CR')||{id:uid(),n:'Camiseta CR',padre:padC.id,ops:conHoja?conHoja.ops:undefined,familiaLMO:conHoja?conHoja.familiaLMO:undefined};
    const padJ=ya('Jogger')?K(ya('Jogger').padre):{id:uid(),n:'JOGGER'};const hJ=ya('Jogger')||{id:uid(),n:'Jogger',padre:padJ.id,minEstConf:12.06};
    [padC,hC,padJ,hJ].forEach(k=>{if(!S.categorias.includes(k)){S.categorias.push(k);creadas.push(k.id)}});const bakSM={c:JSON.stringify(hC.samManda||null),j:JSON.stringify(hJ.samManda||null),jME:hJ.minEstConf};if(hJ.minEstConf==null)hJ.minEstConf=12.06;delete hC.samManda;delete hJ.samManda;const hoja=(samPorCentro(hC)||{}).modulos;
    delete S.params.tiempos21Sembrado;delete S.params.tiempos21Resumen;delete S.params.botonesEstandar;['cordones','apliques','sublimado'].forEach(c=>{const x=CE(c);if(x){delete x.minEstandar;delete x.minEstandarMeta}});
    const nb=S.bitacora.length;sembrarTiempos21();
    __check('T21: la siembra (una vez) deja el SAM de confección que manda por tipo de camiseta (CR 4,47 en vez de la suma de la hoja), corte/empaque de las familias sin hoja (Jogger 0,60/0,69), botones estándar 0,40 y el minuto de cordones/apliques/sublimado, todo «por confirmar» y en bitácora; y no se repite',!!S.params.tiempos21Sembrado&&hC.samManda&&hC.samManda.modulos&&hC.samManda.modulos.min===4.47&&hC.samManda.modulos.pendiente===true&&hJ.samManda&&hJ.samManda.corte.min===0.6&&hJ.samManda.empaque.min===0.69&&prm('botonesEstandar',0)===0.4&&CE('cordones').minEstandar===0.9&&CE('apliques').minEstandar===0.38&&CE('sublimado').minEstandar===1.4&&S.bitacora.length===nb+1&&/Tiempos del 21-sep sembrados/.test(S.bitacora[nb].t)&&(sembrarTiempos21(),S.bitacora.length===nb+1),JSON.stringify({hoja,sm:hC.samManda,j:hJ.samManda,bot:prm('botonesEstandar',0)}));
    const spC=samPorCentro(hC)||{},spJ=samPorCentro(hJ)||{};
    __check('T21: samPorCentro respeta lo que manda: Camiseta CR confección 4,47 (la hoja daba '+num(hoja||0,2)+'); Jogger corte 0,60 y empaque 0,69 con su confección estimada (12,06) intacta',spC.modulos===4.47&&(hoja==null||hoja!==4.47)&&spJ.corte===0.6&&spJ.empaque===0.69&&spJ.modulos===12.06,JSON.stringify({spC,spJ}));
    const tb=tiempoPaso({centro:'botones',t:0},hJ,spJ,0),tc=tiempoPaso({centro:'cordones',t:0},hJ,spJ,0),tl=tiempoPaso({centro:'lavado',t:0},hJ,spJ,0);
    __check('T21: un paso de Botones sin dato en la hoja ni en la tabla toma el estándar 0,40 (marcado estándar); Cordones toma el minuto del centro (0,90); lavado sigue como antes',tb&&tb.t===0.4&&tb.estandar===true&&tc&&tc.t===0.9&&tc.estandar===true&&tl&&typeof tl.t==='number',JSON.stringify({tb,tc,tl}));
    /* editar y confirmar */
    ...
    /* limpieza */
    cerrar();S.ordenes=S.ordenes.filter(x=>x.id!==o.id);S.categorias=S.categorias.filter(k=>!creadas.includes(k.id));{const c=JSON.parse(bakSM.c);if(c)hC.samManda=c;else delete hC.samManda;const j=JSON.parse(bakSM.j);if(j)hJ.samManda=j;else delete hJ.samManda;if(bakSM.jME==null)delete hJ.minEstConf;else hJ.minEstConf=bakSM.jME}
   ...
   {sembrarPasosNuevos20();const ids=['calandrado','sublimado','apliques','cordones'];
    ...
    __check('PS: van en su lugar del proceso: Calandrado antes de Corte, Sublimado y Apliques después de Estampado, Cordones antes de Plancha',ordenPaso('calandrado')<ordenPaso('corte')&&ordenPaso('sublimado')>ordenPaso('estampado')&&ordenPaso('apliques')>ordenPaso('estampado')&&ordenPaso('sublimado')<ordenPaso('modulos')&&ordenPaso('cordones')>ordenPaso('lavado')&&ordenPaso('cordones')<ordenPaso('plancha'));
    __check('PS: la tabla centro → etapa los conoce; Calandrado cuenta por días con una espera POR CONFIRMAR (0 hasta que se cargue); están en «centros por orden» y en los perfiles de corte/estampado y terminados',ids.every(c=>(S.params.centroEtapa||[]).some(x=>x.centro===c))&&centroPorDias('calandrado')&&esperasPaso().some(f=>f.paso==='calandrado'&&f.estimado&&f.dias===0)&&ids.every(c=>centrosPorOrden().includes(c))&&(perfilesDef().find(x=>x.id==='corte')||{centros:[]}).centros.includes('sublimado')&&(perfilesDef().find(x=>x.id==='terminado')||{centros:[]}).centros.includes('cordones'));
    const a1=ordenCentrosAuto({tecnicaTxt:'SUBLIMADO FULL PRINT'}),a2=ordenCentrosAuto({tecnicaTxt:'SERIGRAFIA + APLIQUE'}),a3=ordenCentrosAuto({tecnicaTxt:'SERIGRAFIA'});
    __check('PS: una orden cuya técnica dice sublimado va a Sublimado (no a Estampado); una con aplique agrega Apliques; serigrafía sigue yendo a Estampado',a1.has('sublimado')&&!a1.has('estampado')&&a2.has('estampado')&&a2.has('apliques')&&a3.has('estampado')&&!a3.has('sublimado')&&!a3.has('apliques'),JSON.stringify({a1:[...a1],a2:[...a2],a3:[...a3]}));
    __check('PS: el editor de ruta en lote y el editor por orden ofrecen los cuatro (mismos centros de producción)',ids.every(c=>centrosLote().some(x=>x.id===c))&&ids.every(c=>centrosEditablesRuta().includes(c)));
    const o=S.ordenes.find(x=>abierta(x)&&lanzada(x)&&K(x.cat));if(o){const c=CE('cordones');const bakM=c.minEstandar;const t1=tiempoPaso({centro:'cordones',t:0},K(o.cat),samPorCentro(K(o.cat))||{},0);delete c.minEstandar;const t0=tiempoPaso({centro:'cordones',t:0},K(o.cat),samPorCentro(K(o.cat))||{},0);if(bakM!=null)c.minEstandar=bakM;
     __check('PS: sin minuto en el centro, un paso nuevo entra en 0 (brecha visible), nunca con un valor inventado; con el minuto del centro (Configuración → Centros, sembrado el 21-sep) entra con ese valor marcado estándar',t0&&t0.t===0&&(bakM==null||(t1&&t1.t===+bakM&&t1.estandar===true)),JSON.stringify({bakM,t0,t1}));}}
   ...
   {const H=tareaRows[0];const col=n=>H.indexOf(n);const padre={id:'k_rd_p',n:'FAMILIA SIN HOJA RD'};const hija={id:'k_rd_h',padre:'k_rd_p',n:'Prenda RD',minEstConf:7.5};S.categorias.push(padre,hija);
    ...
    const p=planTarea([tareaRows[0],f,...tareaRows.slice(1,400)],'TAREA_RD.xlsx');const o=p.ordenes.find(x=>x.op==='WH/MO/99301');const cs=o?(o.rutaCompleta||[]).map(x=>x.centro):[];
    __check('RD: una orden nueva de una familia SIN hoja de operaciones nace con la ruta por defecto corte → confección → empaque, marcada «estimada» y con corte y empaque sin tiempo (aviso), confección con el estimado',!!o&&o.rutaDefecto===true&&['corte','modulos','empaque'].every(c=>cs.includes(c))&&cs.indexOf('corte')<cs.indexOf('modulos')&&cs.indexOf('modulos')<cs.indexOf('empaque')&&o.rutaConf&&o.rutaConf.estado==='estimada'&&/hoja de operaciones/.test(o.rutaConf.nota)&&(o.sinTiempo||[]).includes('corte')&&(o.sinTiempo||[]).includes('empaque')&&!(o.sinTiempo||[]).includes('modulos')&&(o.rutaCompleta.find(x=>x.centro==='modulos')||{}).t===7.5,JSON.stringify(o&&{cs,st:o.sinTiempo,rc:o.rutaConf}));
    __check('RD: ninguna orden cuya familia SÍ aporta centros (con hoja) o que no resolvió categoría lleva la marca de ruta por defecto',p.ordenes.every(x=>x.op==='WH/MO/99301'||!x.rutaDefecto||(x.cat&&K(x.cat)&&![...centrosDeCategoria(K(x.cat))].some(c=>CE(c)&&CE(c).area==='pro'))),p.ordenes.filter(x=>x.rutaDefecto).length+' con ruta por defecto');
    S.categorias=S.categorias.filter(k=>k.id!=='k_rd_p'&&k.id!=='k_rd_h');}
  ...
  {const antes=__R.errors.length;const al=window.alert;let av="";window.alert=m=>{av=String(m)};const adminP=PERFIL;
   ...
   {const a=document.querySelector('nav a[data-p="nivelacion"]');
    __check("N1: la nivelación vive en Dirección → «Planificar el mes» (paso 1); Producción ya no tiene entrada propia ni enlace a Configuración",!a&&!!document.querySelector('nav a[data-p="plan"]')&&!document.querySelector('nav a[data-conf="nivel"]'));
   ...
   {const areas=nivUIAreas();const pro=S.centros.filter(c=>c.area==="pro"&&c.activo!==false).length;
    __check("N3: un recuadro por área: Tela, los centros de producción configurados en orden de proceso, Maquila",areas.length===pro+2&&areas[0].id==="tela"&&areas[1].id==="calandrado"&&areas[2].id==="corte"&&areas[areas.length-1].id==="maquila"&&areas.findIndex(a=>a.id==="modulos")>areas.findIndex(a=>a.id==="corte"),areas.map(a=>a.id).join(" → "));
    const h=document.getElementById("p-nivelacion").innerHTML;
    __check("N3: cada recuadro trae saldo, fecha final (o por días / sin saldo) y estado con ícono en su pastilla",areas.every(a=>h.includes("NIVUI.area='"+a.id+"'"))&&(h.match(/class="niv-card /g)||[]).length===areas.length&&(h.match(/>saldo</gi)||[]).length===areas.length-1&&/hay que mandar/.test(h)&&(h.match(/class="niv-pill /g)||[]).length===areas.length&&/[✓⚠✕?] (llega|riesgo|déficit|falta un dato)/.test(h)&&[...document.querySelectorAll("#p-nivelacion .niv-card")].every(c=>c.dataset.area==="maquila"||c.classList.contains("vacia")||c.classList.contains("est-dias")||/termina|fin —/.test(c.textContent)));
    ...
    __check("N3: clic en un recuadro muestra SOLO esa área abajo",/Nivelación · Confección/.test(h2)&&!/Nivelación · Corte/.test(h2)&&(h2.match(/Saldo por familia y mes/g)||[]).length===1);
   ...
   {const tabla=faseMapeo();const bak=tabla.map(r=>r.nivel);
    ...
    __check("N8: Maquila ya no se cuenta por fase en el recuadro: es el descarte de Confección (dice cuánto hay que mandar o «no hace falta»), y sin fases marcadas el cálculo por fase sigue diciendo faltante por debajo",(fasesDeProcNivel("maquila").length>0||(rM0.sinFasesTela===true&&rM0.estado==="faltante"))&&/hay que mandar/.test(document.querySelector("#p-nivelacion .niv-card[data-area=maquila]").textContent)&&/enviar|no hace falta|falta un dato/.test(document.querySelector("#p-nivelacion .niv-card[data-area=maquila]").textContent));   /* la tarjeta dice «sin fases marcadas» + «? falta un dato · falta marcar en la tabla 1 qué fases cuentan» */
  ...
  {const antes=__R.errors.length;const al=window.alert;const alerts=[];window.alert=m=>{alerts.push(String(m))};PERFIL=adminP0();
   ...
   /* la descarga local se observa sin abrir ventanas */
   const dj=window.descargarJSON;let local=null;window.descargarJSON=(n,o)=>{local={n,ordenes:(o&&o.ordenes||[]).length,params:!!(o&&o.params)}};
   /* se siembra de todo un poco para ver qué sobrevive y qué se limpia (claves REALES, no inventadas) */
   ...
   S.params.metas=Object.assign(S.params.metas||{},{"2026-12":123456});S.params.progTej=(S.params.progTej||[]).concat([{id:"pt-b0",tela:(S.telas[0]||{}).id,dia:"2026-09-01",kg:10,estado:"tejido",kgReal:9}]);
   const oA=S.ordenes.find(o=>abiertaDe(o)&&o.op);
   S.params.pendPospuestos=Object.assign(S.params.pendPospuestos||{},{sinFecha:{hasta:"2026-12-31",motivo:"p",n:5}});   /* la clave real es la de la bandeja */
   S.params.planMes=Object.assign(S.params.planMes||{},{"2026-12":{oids:[oA.id],congelado:{ver:1,ts:"x",u:"p"}}});S.params.progCongelado=(S.params.progCongelado||[]).concat([{id:"pc-b0",centro:"corte",lun:"2026-09-14",oids:[oA.id],hechasAl:{}}]);
   S.params.alertasCompras=(S.params.alertasCompras||[]).concat([{id:"ac-b0",oid:oA.id,op:oA.op}]);S.params.advertencias=(S.params.advertencias||[]).concat([{id:"adv-b0",ts:"x",u:"p",oid:oA.id,op:oA.op,accion:"p",atendida:false}]);
   S.params.auditoriaCambios=(S.params.auditoriaCambios||[]).concat([{ts:"x",u:"p",tipo:"fase",oid:oA.id,op:oA.op}]);
   ...
   const fotoDe=()=>({centros:S.centros.length,recursos:S.recursos.map(r=>[r.id,r.pers,r.efic,r.min]).sort().join("|"),cal:JSON.stringify(S.params.cal||null),exc:(S.params.excepciones||[]).length,
     ...
     bucket:Object.keys((sb.storage.__FILES||{})).filter(k=>!/^respaldos\//.test(k)).length,esc:JSON.stringify((S.params.nivelacion||{}).escenarios),categorias:S.categorias.length,tecnicas:S.tecnicas.length,rutas:(S.rutas||[]).length,maquinas:(S.maquinas||[]).length,
     metas:JSON.stringify(S.params.metas||null),progTej:JSON.stringify(S.params.progTej||null),cierresMes:JSON.stringify(S.params.cierresMes||null),restauraciones:(S.params.restauraciones||[]).length});
   const foto=fotoDe();const bitAntes=S.bitacora.length;const c0=cuentaBorrado();const audAntes=(S.params.auditoriaCambios||[]).length;
   const huerfanosDe=()=>({pendPospuestos:Object.keys(S.params.pendPospuestos||{}).length,planMesOids:Object.values(S.params.planMes||{}).reduce((a,m)=>a+((m.oids||[]).length),0),planMesCongelados:Object.values(S.params.planMes||{}).filter(m=>m&&m.congelado).length,progCongelado:(S.params.progCongelado||[]).length,
     alertasCompras:(S.params.alertasCompras||[]).length,advertencias:(S.params.advertencias||[]).length,noCalzan:(((S.params.tareaCarga||{}).recarga||{}).noCalzan||[]).length,tareaCarga:!!S.params.tareaCarga,otCarga:!!S.params.otCarga,pedidosReprog:(Array.isArray(S.params.pedidosReprog)?S.params.pedidosReprog.length:0),fotosIdx:Object.keys(S.params.fotosIdx||{}).length});
   const h0=huerfanosDe();
   __check("B0: el volcado supera las 1.000 filas de una consulta (el mock ahora corta como PostgREST): el borrado tiene que paginar",c0.ordenes>1000,c0.ordenes);
   /* --- un solo camino --- */
   {page="config";CONF.tab="borrado";render();const h=document.getElementById("p-config").innerHTML;
    __check("B1: hay UN solo botón «Borrar datos de prueba (conserva la configuración)» y «Quitar todas las órdenes» ya no existe",/Borrar datos de prueba \(conserva la configuración\)/.test(h)&&!/Quitar todas las órdenes/.test(h)&&(h.match(/onclick="mBorrar\(/g)||[]).length===1&&typeof window.ejecutarBorrado==="function"&&/^async function ejecutarBorrado\(\)/.test(String(ejecutarBorrado).replace(/\s+/g," ").slice(0,40)));
    __check("B1: el panel dice qué se limpia en Configuración (con las advertencias de fecha) y qué se conserva (auditoría, bitácora, cierres de mes)",/bandejas pospuestas/.test(h)&&/advertencias de fecha/.test(h)&&/bandeja «no calzan»/.test(h)&&/auditoría y cierres de mes/.test(h));
    mBorrar();const m=document.getElementById("modal").innerHTML;
    __check("B1: la confirmación es la palabra BORRAR, con el detalle y los conteos de lo que se borra y lo que se conserva, la casilla del índice de fotos desmarcada, el botón Borrar llama a ejecutarBorrado() y avisa de las demás sesiones",/escribe exactamente <b>BORRAR<\/b> en este recuadro/.test(m)&&/placeholder="escribe BORRAR aquí"/.test(m)&&/onkeydown="if\(event.key==='Enter'\)\{event.preventDefault\(\);ejecutarBorrado\(\)\}"/.test(m)&&/Se borra/.test(m)&&/Se conserva/.test(m)&&new RegExp("Órdenes: <b>"+num(c0.ordenes)+"</b>").test(m)&&new RegExp("Registros de avance de piso[^<]*<b>"+num(c0.avanceReg)+"</b>").test(m)&&/bandejas pospuestas/.test(m)&&/Auditoría de cambios/.test(m)&&/auditoría registrada desde el piso/.test(m)&&/Cierres de mes/.test(m)&&/tejeduría/.test(m)&&!!document.getElementById("borrar-fotosidx")&&document.getElementById("borrar-fotosidx").checked===false&&/no se borra nada/.test(m)&&/onclick="ejecutarBorrado\(\)"/.test(m)&&/Cierra las demás sesiones/.test(m),m.slice(0,200));
    document.getElementById("borrar-frase").value="borrar todo";const okMal=await ejecutarBorrado();
    __check("B1: con otra palabra no se borra nada y el aviso dice qué se escribió",okMal===false&&S.ordenes.length===c0.ordenes&&/Escribiste «BORRAR TODO» y la palabra es BORRAR\. No se borró nada/.test(alerts[alerts.length-1]||""));
    document.getElementById("borrar-frase").value="";const okVacio=await ejecutarBorrado();
    __check("B1: con el recuadro vacío no se borra nada y el aviso dice dónde está el recuadro",okVacio===false&&S.ordenes.length===c0.ordenes&&/El recuadro está vacío: escribe BORRAR en el recuadro que está encima de los botones/.test(alerts[alerts.length-1]||""));
    await __p(30);__check("B1: al abrir el modal el cursor queda en el recuadro de la palabra",document.activeElement&&document.activeElement.id==="borrar-frase");cerrar();
    {const bakP=PERFIL;PERFIL={id:"u-piso",rol:"corte",nombre:"Sup",modo:"editar"};document.getElementById("modal").innerHTML="";const inp=document.createElement("input");inp.id="borrar-frase";inp.value="BORRAR";document.body.appendChild(inp);const okP=await ejecutarBorrado();inp.remove();PERFIL=bakP;
     __check("B1: sin permiso config ni ejecutarBorrado() borra (aunque escriban la palabra)",okP===false&&S.ordenes.length===c0.ordenes&&/administrador/.test(alerts[alerts.length-1]||""));}}
   const nFiles=()=>Object.keys(sb.storage.__FILES||{}).filter(k=>/^respaldos\/respaldos\/respaldo_antes_de_borrar_/.test(k)).length;
   const nDel=()=>__W.writes.filter(w=>w.op==="delete").reduce((a,w)=>a+w.n,0);
   const dispara=async()=>{document.getElementById("modal").innerHTML="";const inp=document.createElement("input");inp.id="borrar-frase";inp.value="BORRAR";document.body.appendChild(inp);while(guardando)await __p(20);const ok=await ejecutarBorrado();while(guardando)await __p(20);await __p(80);inp.remove();return ok};
   /* --- si no se puede guardar antes, no se borra --- */
   {S.params.msBuscar=223;__W.deny=(t,op)=>t==="params"&&op==="upsert";const d0=nDel();const ok=await dispara();__W.deny=null;
    __check("B2: si el guardado previo falla (params sin permiso), NO se borra nada y queda CANCELADO",ok===false&&nDel()===d0&&S.ordenes.length===c0.ordenes&&/no se pudo guardar antes/.test(alerts[alerts.length-1]||"")&&!BORRANDO&&!document.getElementById("borrando-velo"),alerts[alerts.length-1]);
    S.params.msBuscar=222;while(guardando)await __p(20);await save();while(guardando)await __p(20);}
   /* --- si el respaldo no sube, no se borra --- */
   {const st=sb.storage;const bakFrom=st.from;st.from=()=>({upload:async()=>({data:null,error:{message:"bucket no existe"}}),download:async()=>({data:null,error:{message:"no"}})});
    const d0=nDel();local=null;const ok=await dispara();st.from=bakFrom;
    __check("B2: si el respaldo no se puede subir al servidor, NO se borra nada (ningún delete), queda en bitácora como CANCELADO y la descarga local sí se pidió",ok===false&&nDel()===d0&&S.ordenes.length===c0.ordenes&&Object.keys(S.avance).length===c0.avanceReg&&/BORRADO CANCELADO: el respaldo no se pudo subir/.test((S.bitacora.slice(-3).map(b=>b.t||"").join(" ")))&&/NO se borró nada/.test(alerts[alerts.length-1]||"")&&!!local&&/respaldo_antes_de_borrar_/.test(local.n)&&local.ordenes===c0.ordenes&&local.params,alerts[alerts.length-1]);}
   /* --- si el respaldo del servidor no se puede leer como lo haría Restaurar, no se borra (tres ramas) --- */
   {const st=sb.storage;const bakFrom=st.from;const casos=[["estructura",'{"hola":1}',/no pasa la validación de Restaurar/],["no JSON","esto no es json",/no es JSON legible/],["conteo",JSON.stringify({params:{},ordenes:[{id:"x"}]}),/trae 1 órdenes y hay/]];
    for(const [nom,txt,re] of casos){st.from=(b)=>{const o=bakFrom.call(st,b);o.download=async()=>({data:new Blob([txt],{type:"application/json"}),error:null});return o};
     const antesF=nFiles();const d0=nDel();const ok=await dispara();st.from=bakFrom;
     __check("B2: respaldo del servidor inválido ("+nom+") → NO se borra nada (ningún delete), el local y el del servidor sí se dejaron, y la bitácora dice por qué",ok===false&&nDel()===d0&&S.ordenes.length===c0.ordenes&&nFiles()===antesF+1&&re.test((S.bitacora.slice(-2).map(b=>b.t||"").join(" "))),alerts[alerts.length-1]);}}
   __check("B2: esRespaldoValido es el mismo criterio de Restaurar",/esRespaldoValido\(nuevo\)/.test(String(restaurarDesde))&&esRespaldoValido({params:{},ordenes:[]})&&!esRespaldoValido({params:{}})&&!esRespaldoValido({ordenes:[]})&&!esRespaldoValido(null));
   /* --- si una tabla falla a mitad (error del servidor): se para, queda INCOMPLETO, Configuración no se limpia, se puede deshacer --- */
   {const copia2=JSON.stringify(S);__W.deny=(t,op)=>t==="banos_conf"&&op==="delete";const antesF=nFiles();const ok=await dispara();__W.deny=null;
    const inc=S.params.borradoIncompleto;const modal=document.getElementById("modal").innerHTML;
    __check("B3: las órdenes van al final y el fallo en baños se para ahí: queda S.params.borradoIncompleto con lo borrado, el fallo, lo pendiente y el respaldo; el modal lo dice, ofrece «Restaurar el respaldo» y la bitácora también",ok===false&&!!inc&&inc.fallo&&inc.fallo.t==="banos_conf"&&(inc.borradas||[]).map(x=>x.t).join()==="avance,banos_conf"&&(inc.pendientes||[]).join()==="banos_conf,salidas_tin,programas,cargas,propuestas,paros,turnos,planes,ordenes"&&/respaldo_antes_de_borrar_/.test(inc.respaldo)&&/^respaldos\/respaldos\//.test(inc.rutaServidor)&&/Borrado INCOMPLETO/.test(modal)&&/restaurarRespaldoServidor\(/.test(modal)&&/BORRADO INCOMPLETO/.test(S.bitacora.slice(-1)[0].t||"")&&nFiles()===antesF+1&&!S.params.borradoEnCurso,JSON.stringify(inc).slice(0,300));
    __check("B3: lo que sigue después de la tabla fallida NO se tocó (órdenes intactas: sin avances huérfanos) y Configuración no se limpió",S.ordenes.length===c0.ordenes&&Object.keys(S.avance).length===0&&(S.banos_conf||[]).length===c0.banos&&(S.cargas||[]).length===c0.cargas&&(S.turnos||[]).length===c0.turnos&&(S.planes||[]).length===c0.planes&&JSON.stringify(huerfanosDe())===JSON.stringify(h0),JSON.stringify(huerfanosDe()));
    cerrar();page="config";CONF.tab="borrado";render();
    __check("B3: el panel avisa del borrado incompleto con qué falló y qué queda, y ofrece restaurar",/último borrado quedó INCOMPLETO/.test(document.getElementById("p-config").innerHTML)&&/baños confirmados/.test(document.getElementById("p-config").innerHTML)&&/restaurarRespaldoServidor\(/.test(document.getElementById("p-config").innerHTML));
    /* deshacer de verdad: Restaurar el respaldo del servidor devuelve el avance borrado */
    {const pr=window.prompt;window.prompt=()=>"RESTAURAR";const okR=await restaurarRespaldoServidor(inc.rutaServidor);window.prompt=pr;while(guardando)await __p(20);
     __check("B3: «Restaurar el respaldo (deshacer)» baja el respaldo del servidor y lo carga con Restaurar: vuelven las órdenes y el avance",okR===true&&S.ordenes.length===c0.ordenes&&Object.keys(S.avance).length===c0.avanceReg,S.ordenes.length+" / "+Object.keys(S.avance).length);}
    /* se vuelve al estado sembrado (la base simulada también) conservando la marca de incompleto, para que el borrado completo la limpie */
    S=JSON.parse(copia2);S.params.borradoIncompleto=inc;TABLAS.forEach(t=>{BASE[t]={}});BASE.params=null;while(guardando)await __p(20);await save();while(guardando)await __p(20);await __p(50);}
   /* --- RLS silencioso: el delete no da error pero no borra → se detecta releyendo y queda INCOMPLETO --- */
   {const copia3=JSON.stringify(S);__W.silencio=t=>t==="avance";const ok=await dispara();__W.silencio=null;const inc=S.params.borradoIncompleto;
    __check("B3: un delete que el servidor ignora (RLS sin política DELETE, sin error) se detecta releyendo: INCOMPLETO en avance con «el servidor no borró», nada más se tocó",ok===false&&!!inc&&inc.fallo.t==="avance"&&/el servidor no borró/.test(inc.fallo.error)&&S.ordenes.length===c0.ordenes&&Object.keys(S.avance).length===c0.avanceReg,JSON.stringify(inc&&inc.fallo));
    cerrar();S=JSON.parse(copia3);TABLAS.forEach(t=>{BASE[t]={}});BASE.params=null;while(guardando)await __p(20);await save();while(guardando)await __p(20);await __p(50);}
   /* --- candados: durante el borrado no se guarda nada ajeno; con la carga a medias tampoco --- */
   {const w0=__W.writes.length;BORRANDO=true;await save();BORRANDO=false;const w1=__W.writes.length;CARGA_INCOMPLETA="centros";await save();CARGA_INCOMPLETA=null;
    __check("B3: con BORRANDO un save() ajeno no escribe nada, y con CARGA_INCOMPLETA tampoco (hasta recargar la página)",w1===w0&&__W.writes.length===w0&&/CARGA_INCOMPLETA/.test(String(save))&&/CARGA_INCOMPLETA=t/.test(String(cargarTodo))&&/return true/.test(String(cargarTodo)));}
   /* --- el borrado completo --- */
   {const antesF=nFiles();const bitN=S.bitacora.length;const w0=__W.writes.length;local=null;const ok=await dispara();
    const despues=fotoDe();const dif=Object.keys(foto).filter(k=>foto[k]!==despues[k]);
    const ws=__W.writes.slice(w0);const iUp=ws.findIndex(w=>w.op==="upsert");const iDel=ws.findIndex(w=>w.op==="delete");
    __check("B4: el borrado termina (las diez tablas, más de 1.000 órdenes, verificadas en cero), guardó ANTES del primer delete, pidió la descarga local con las órdenes que había y dejó una sola copia nueva en el servidor",ok===true&&/BORRADO de datos de prueba TERMINADO/.test((S.bitacora.slice(-1)[0]||{}).t||"")&&nFiles()===antesF+1&&iUp>=0&&iDel>iUp&&!!local&&local.ordenes===c0.ordenes&&!BORRANDO&&!document.getElementById("borrando-velo"),JSON.stringify({iUp,iDel,local}));
    __check("B4: se conserva TODA la configuración (centros, recursos, calendario y festivos, tabla 1 con nivelación, 14, 15, 18, grupos, telas, colores, operaciones, parámetros, usuarios, rutas por defecto, metas, tejeduría manual, escenarios) y también cierres de mes, restauraciones y los archivos de fotos del bucket",dif.length===0,dif.map(k=>k+": "+String(foto[k]).slice(0,40)+" → "+String(despues[k]).slice(0,40)).join(" | "));
    __check("B4: se borran órdenes, avance, planes, baños, salidas, programas, cargas, propuestas, turnos y paros; la bitácora NO (y crece); INICIADO y TERMINADO quedan en bitácora",S.ordenes.length===0&&Object.keys(S.avance).length===0&&(S.planes||[]).length===0&&(S.banos_conf||[]).length===0&&(S.salidas_tin||[]).length===0&&(S.programas||[]).length===0&&(S.cargas||[]).length===0&&(S.propuestas||[]).length===0&&(S.turnos||[]).length===0&&(S.paros||[]).length===0&&S.bitacora.length>bitAntes&&S.bitacora.some(b=>/INICIADO por/.test(b.t||""))&&S.bitacora.some(b=>/TERMINADO por/.test(b.t||"")),JSON.stringify({o:S.ordenes.length,a:Object.keys(S.avance).length,b:S.bitacora.length}));
    const huer=huerfanosDe();
    __check("B0→B4: ya NO quedan huérfanos en params (bandejas pospuestas, borrador y congelado del plan, congelados semanales, alertas de compras, advertencias de fecha, «no calzan», resúmenes de carga, pedidos de reprogramación), el índice de fotos se vació por defecto, y los meses del plan, las metas y la tejeduría manual quedan",huer.pendPospuestos+huer.planMesOids+huer.planMesCongelados+huer.progCongelado+huer.alertasCompras+huer.advertencias+huer.noCalzan+huer.pedidosReprog+huer.fotosIdx===0&&!huer.tareaCarga&&!huer.otCarga&&!!(S.params.planMes||{})["2026-12"]&&(S.params.metas||{})["2026-12"]===123456&&(S.params.progTej||[]).some(p=>p.id==="pt-b0")&&!S.params.borradoIncompleto&&!S.params.borradoEnCurso,JSON.stringify(huer));
    __check("B4: la auditoría que el piso escribió dentro de avance se copió a la auditoría de Configuración antes de borrar (con oid y marca), y la auditoría previa sigue",(S.params.auditoriaCambios||[]).length===audAntes+1&&(S.params.auditoriaCambios||[]).some(e=>e.oid===oA.id&&e.conservadaEnBorrado&&e.tipo==="cierre"),(S.params.auditoriaCambios||[]).length+" vs "+audAntes);
    const reg=(S.params.borrados||[]).slice(-1)[0]||{};const bit=(S.bitacora.slice(-1)[0]||{});const bt=bit.t||"";
    __check("B5: bitácora con usuario, fecha, conteos, huérfanos limpiados, ruta del respaldo (local y servidor) y el respaldo completo del intento anterior",new RegExp("TERMINADO por "+quienFirma().replace(/[.*+?^$()|[\]\\]/g,"\\$&")).test(bt)&&/ el 20\d\d-\d\d-\d\d \d\d:\d\d/.test(bt)&&new RegExp("órdenes "+num(c0.ordenes)).test(bt)&&/bandejas pospuestas/.test(bt)&&/respaldo_antes_de_borrar_[^ ]+\.json \(local\) y respaldos\/respaldos\/respaldo_antes_de_borrar_[^ ]+ \(servidor\)/.test(bt)&&/respaldo del intento anterior: respaldos\/respaldos\//.test(bt),bt.slice(0,300));
    __check("B5: S.params.borrados registra el borrado (quién, cuándo, conteos, huérfanos, auditoría copiada, respaldo local, ruta del servidor y el respaldo previo del intento incompleto)",!!reg.ts&&reg.u===quienFirma()&&reg.conteos&&reg.conteos.ordenes===c0.ordenes&&reg.huerfanos&&reg.huerfanos.pendPospuestos===h0.pendPospuestos&&reg.huerfanos.advertencias===h0.advertencias&&reg.audPiso===1&&/respaldo_antes_de_borrar_/.test(reg.respaldo)&&/^respaldos\/respaldos\//.test(reg.rutaServidor)&&reg.conservarFotosIdx===false&&reg.respaldoPrevio&&/^respaldos\/respaldos\//.test(reg.respaldoPrevio.rutaServidor),JSON.stringify(reg).slice(0,300));
    const modal=document.getElementById("modal").innerHTML;
    __check("B6: al terminar sale «Base vacía · configuración conservada» con los conteos de lo conservado (centros, recursos, telas, colores, operaciones, categorías, fases, motivos, ventanas, grupos, usuarios, tejeduría manual, fotos del bucket sin índice, parámetros) y el enlace a Actualizar datos",/Base vacía · configuración conservada/.test(modal)&&/verificadas en cero/.test(modal)&&new RegExp("Centros</td><td class=\"num\"><b>"+num(S.centros.length)).test(modal)&&/Telas<\/td>/.test(modal)&&/Operaciones \(hoja LMO\)/.test(modal)&&/Fases de la tabla 1/.test(modal)&&/Motivos \(tabla 15\)/.test(modal)&&/Ventanas de descanso/.test(modal)&&/Grupos de módulos/.test(modal)&&/Usuarios<\/td><td class="num"><b>1<\/b>/.test(modal)&&/Programación manual de tejeduría/.test(modal)&&/sin índice no se muestran/.test(modal)&&/Parámetros en Configuración/.test(modal)&&/mActualizarDatos\(1\)/.test(modal),modal.slice(0,200));
    cerrar();page="config";CONF.tab="borrado";render();
    __check("B6: el panel muestra el último borrado (fecha, quién, órdenes, respaldo y el del intento anterior)",/Último borrado:/.test(document.getElementById("p-config").innerHTML)&&new RegExp(num(c0.ordenes)+" órdenes").test(document.getElementById("p-config").innerHTML)&&/respaldo del intento anterior/.test(document.getElementById("p-config").innerHTML));
    /* el respaldo del servidor se puede restaurar de verdad */
    {const r=await sb.storage.from(respaldosBucket()).download("respaldos/"+reg.respaldo);const obj=JSON.parse(await r.data.text());const pr=window.prompt;window.prompt=()=>"RESTAURAR";const okR=await restaurarDesde(obj,reg.respaldo);window.prompt=pr;while(guardando)await __p(20);
     __check("B2/B6: el respaldo que quedó en el servidor se carga con Restaurar y devuelve las órdenes que había",okR===true&&S.ordenes.length===c0.ordenes&&(S.params.restauraciones||[]).length>=1,S.ordenes.length+" vs "+c0.ordenes);}}
   /* --- borrado interrumpido: la marca «en curso» se guardó antes de borrar y el panel la muestra en otra sesión --- */
   {S.params.borradoEnCurso={ts:"2026-09-17T20:00:00.000Z",u:"Admin",respaldo:"respaldo_antes_de_borrar_x.json",rutaServidor:"respaldos/respaldos/respaldo_antes_de_borrar_x.json"};page="config";CONF.tab="borrado";render();
    __check("B7: si la pestaña se cerró a mitad, la marca «borrado en curso» (guardada antes del primer delete) hace que el panel avise «INTERRUMPIDO» con el respaldo y el enlace para restaurar",/borrado INTERRUMPIDO/.test(document.getElementById("p-config").innerHTML)&&/respaldo_antes_de_borrar_x/.test(document.getElementById("p-config").innerHTML)&&/restaurarRespaldoServidor\(/.test(document.getElementById("p-config").innerHTML)&&/borradoEnCurso=\{ts/.test(String(ejecutarBorrado)));delete S.params.borradoEnCurso;}
   /* --- con la casilla marcada, el índice de fotos se conserva --- */
   {S.params.fotosIdx=Object.assign(S.params.fotosIdx||{},{"whx":{ts:1,b:"x"}});while(guardando)await __p(20);await save();while(guardando)await __p(20);document.getElementById("modal").innerHTML="";
    const inp=document.createElement("input");inp.id="borrar-frase";inp.value="BORRAR";document.body.appendChild(inp);const cb=document.createElement("input");cb.type="checkbox";cb.id="borrar-fotosidx";cb.checked=true;document.body.appendChild(cb);
    while(guardando)await __p(20);const ok=await ejecutarBorrado();while(guardando)await __p(20);await __p(80);inp.remove();cb.remove();cerrar();
    __check("B4: con la casilla marcada el índice de fotos se conserva y queda dicho en el registro",ok===true&&Object.keys(S.params.fotosIdx||{}).length>=1&&(S.params.borrados||[]).slice(-1)[0].conservarFotosIdx===true);}
   __check("GUARDIA: las funciones del borrado siguen exigiendo config y la palabra escrita, y la limpieza de params también pide config",/puede\('config'\)/.test(String(mBorrar))&&/puede\('config'\)/.test(String(ejecutarBorrado))&&/PALABRA_BORRAR/.test(String(ejecutarBorrado))&&/puede\('config'\)/.test(String(borrarOperativo))&&/puede\('config'\)/.test(String(limpiarHuerfanosTrasBorrado))&&/puede\('config'\)/.test(String(restaurarRespaldoServidor)));
