...
async function __run(){try{LISTO=true;}catch(e){}try{__R.prevFuzz=localStorage.__fuzz||'';__R.prevFase=localStorage.__fase||'';localStorage.__fuzz='';localStorage.__fase=''}catch(e){}
  ...
  {const antes=__R.errors.length;const adminP=PERFIL;
   ...
   const nO=S.ordenes.length;mBorrar();const fr=document.getElementById('borrar-frase');__check("Borrado: pide la palabra BORRAR escrita, con el conteo de órdenes a la vista",!!fr&&document.getElementById('modal').innerHTML.includes('escribe exactamente: <b>BORRAR</b>')&&document.getElementById('modal').innerHTML.includes('Órdenes: <b>'+num(nO)+'</b>'));
   const alertPrev=window.alert;let al='';window.alert=m=>al=m;fr.value='borrar todo';await ejecutarBorrado();__check("Borrado: palabra incorrecta no borra nada",S.ordenes.length===nO&&/no coincide/.test(al));window.alert=alertPrev;cerrar();
  ...
  {const antes=__R.errors.length;const al=window.alert;const alerts=[];window.alert=m=>{alerts.push(String(m))};PERFIL=adminP0();
   ...
   {abrir('corte',1);const P=programar();const lun=lunesDe(dsum(h,7)),dom=dsum(lun,6);
    ...
    __check("RC2: Listas + En espera = la cola del centro (colaCentro) acotada a órdenes de planta, sin repetir ni perder ninguna",listas.length+espera.length===cola.length&&ids(listas.concat(espera))===ids(cola)&&new Set(listas.map(f=>f.o.id)).size===listas.length);
  ...
  {const antes=__R.errors.length;const al=window.alert;const alerts=[];window.alert=m=>{alerts.push(String(m))};PERFIL=adminP0();
   ...
   {page="config";CONF.tab="borrado";render();const h=document.getElementById("p-config").innerHTML;
    ...
    __check("B1: la confirmación es la palabra BORRAR, con el detalle y los conteos de lo que se borra y lo que se conserva, la casilla del índice de fotos desmarcada, el botón Borrar llama a ejecutarBorrado() y avisa de las demás sesiones",/escribe exactamente: <b>BORRAR<\/b>/.test(m)&&/Se borra/.test(m)&&/Se conserva/.test(m)&&new RegExp("Órdenes: <b>"+num(c0.ordenes)+"</b>").test(m)&&new RegExp("Registros de avance de piso[^<]*<b>"+num(c0.avanceReg)+"</b>").test(m)&&/bandejas pospuestas/.test(m)&&/Auditoría de cambios/.test(m)&&/auditoría registrada desde el piso/.test(m)&&/Cierres de mes/.test(m)&&/tejeduría/.test(m)&&!!document.getElementById("borrar-fotosidx")&&document.getElementById("borrar-fotosidx").checked===false&&/no se borra nada/.test(m)&&/onclick="ejecutarBorrado\(\)"/.test(m)&&/Cierra las demás sesiones/.test(m),m.slice(0,200));
    document.getElementById("borrar-frase").value="borrar todo";const okMal=await ejecutarBorrado();
    __check("B1: con otra palabra no se borra nada",okMal===false&&S.ordenes.length===c0.ordenes&&/no coincide/.test(alerts[alerts.length-1]||""));cerrar();
